...
let openCentralWishId='';
function toggleCentralWish(id){openCentralWishId=String(openCentralWishId)===String(id)?'':String(id);renderWishlists()}
window.toggleCentralWish=toggleCentralWish;
function centralWishRow(w){const me=currentUserName(),own=Boolean(me&&samePerson(me,w.owner)),href=/^https?:\/\//i.test(String(w.link||'').trim())?String(w.link||'').trim():'',open=String(openCentralWishId)===String(w.id);return `<article class="central-wish-row ${open?'open':''}" onclick="toggleCentralWish('${esc(w.id)}')"><div class="wish-row-summary">${w.image?`<button class="wish-image-button" type="button" onclick="event.stopPropagation();openWishImage('${esc(w.id)}')"><img src="${esc(w.image)}" alt="Afbeelding van ${esc(w.text)}"></button>`:'<div class="wish-image-placeholder">🎁</div>'}<div class="central-wish-main"><div><strong>${esc(w.text||'')}</strong>${w.price?`<span class="wish-price">${money(String(w.price).replace(',','.'))}</span>`:''}</div>${w.variant?`<small>${esc(w.variant)}</small>`:''}</div><span class="wish-chevron">${open?'⌃':'⌄'}</span></div>${open?`<div class="wish-extra" onclick="event.stopPropagation()">${w.link?`<div>${href?`<a href="${esc(href)}" target="_blank" rel="noopener">Productlink openen ↗</a>`:esc(w.link)}</div>`:''}${w.memo?`<div><strong>Extra informatie</strong><br>${esc(w.memo)}</div>`:''}${!w.link&&!w.memo&&!w.variant?'<div class="wish-no-extra">Geen extra informatie toegevoegd.</div>':''}${!own&&me?`<label class="wish-status-label">Status<select onchange="updateCentralWishStatus('${esc(w.id)}',this.value)">${[['free','Vrij'],['reserved','Gereserveerd'],['bought','Gekocht'],['wrapped','Ingepakt']].map(([v,l])=>`<option value="${v}" ${w.status===v?'selected':''}>${l}</option>`).join('')}</select></label>`:''}${own?`<div class="central-wish-actions"><button type="button" onclick="editCentralWish('${esc(w.id)}')">Wijzigen</button><button type="button" class="danger" onclick="deleteCentralWish('${esc(w.id)}')">Verwijderen</button></div>`:''}</div>`:''}</article>`}
function renderWishlists(){const list=$('#list'),groups=new Map(),editing=wishlists.find(w=>String(w.id)===String(editingCentralWishId)),formOpen=Boolean(editingCentralWishId||window.hcWishFormOpen);wishlists.forEach(w=>{const owner=String(w.owner||'Onbekend').trim()||'Onbekend';if(!groups.has(owner))groups.set(owner,[]);groups.get(owner).push(w)});const ownerValue=editing?.owner||currentUserName(),textValue=editing?.text||'',priceValue=editing?String(editing.price||'').replace('.',','):'',linkValue=editing?.link||'',variantValue=editing?.variant||'',memoValue=editing?.memo||'';list.innerHTML=`<section class="wishlist-page"><div class="wishlist-head"><div><h2>Verlanglijstjes</h2><p>Bekijk de wensen per persoon.</p></div><button class="primary compact-wish-add" type="button" onclick="openCentralWishForm()">+ Wens toevoegen</button></div>${[...groups.entries()].sort((a,b)=>a[0].localeCompare(b[0],'nl')).map(([owner,rows])=>`<section class="central-wish-person"><h3>${esc(owner)} <small>${rows.length}</small></h3>${rows.sort((a,b)=>(b.createdAt||0)-(a.createdAt||0)).map(centralWishRow).join('')}</section>`).join('')||'<div class="empty"><h2>Nog geen wensen</h2><p>Voeg de eerste wens toe.</p></div>'}${formOpen?`<section class="central-wish-add" id="centralWishFormSection"><h3>${editing?'Wens wijzigen':'Wens toevoegen'}</h3><div class="central-wish-form"><input id="centralWishOwner" placeholder="Voor wie?" value="${esc(ownerValue)}" ${editing?'readonly':''}><input id="centralWishText" placeholder="Cadeauwens" value="${esc(textValue)}"><input id="centralWishPrice" inputmode="decimal" placeholder="Richtprijs €" value="${esc(priceValue)}"><input id="centralWishLink" placeholder="Productlink (optioneel)" value="${esc(linkValue)}"><input id="centralWishVariant" placeholder="Maat, kleur of uitvoering" value="${esc(variantValue)}"><textarea id="centralWishMemo" rows="2" placeholder="Memo (optioneel)">${esc(memoValue)}</textarea>${editing?.image?`<div class="wish-current-image"><img src="${esc(editing.image)}" alt="Huidige afbeelding"><span>Huidige afbeelding</span></div>`:''}<label class="wish-file-label">${editing?.image?'Andere afbeelding kiezen (optioneel)':'Afbeelding (optioneel)'}<input id="centralWishImage" type="file" accept="image/*"></label><div class="central-wish-form-actions"><button class="primary" type="button" onclick="addCentralWish()">${editing?'Wijzigingen opslaan':'Wens opslaan'}</button><button type="button" onclick="cancelCentralWishEdit()">Annuleren</button></div></div></section>`:''}</section>`}
...
window.openCentralWishForm=()=>{window.hcWishFormOpen=true;editingCentralWishId='';renderWishlists();requestAnimationFrame(()=>$('#centralWishFormSection')?.scrollIntoView({behavior:'smooth',block:'nearest'}))};
window.addCentralWish=async()=>{const owner=$('#centralWishOwner')?.value.trim(),text=$('#centralWishText')?.value.trim();if(!owner||!text){alert('Vul voor wie de wens is en de cadeauwens in.');return}const existing=wishlists.find(x=>String(x.id)===String(editingCentralWishId));let image=existing?.image||'';const file=$('#centralWishImage')?.files?.[0];if(file){try{image=await compressWishImage(file)}catch(_){alert('De afbeelding kon niet worden toegevoegd.');return}}const values={owner,text,price:$('#centralWishPrice')?.value.trim().replace(',','.')||'',link:$('#centralWishLink')?.value.trim()||'',variant:$('#centralWishVariant')?.value.trim()||'',memo:$('#centralWishMemo')?.value.trim()||'',image};if(existing){if(!samePerson(currentUserName(),existing.owner))return;Object.assign(existing,values);editingCentralWishId=''}else{values.owner=currentUserName()||owner;wishlists.push({id:crypto.randomUUID?crypto.randomUUID():String(Date.now()),...values,ownerUid:occasionUser?.uid||'',status:'free',createdAt:Date.now()})}window.hcWishFormOpen=false;save()};
window.editCentralWish=id=>{const w=wishlists.find(x=>String(x.id)===String(id));if(!w||!samePerson(currentUserName(),w.owner))return;editingCentralWishId=String(id);renderWishlists();requestAnimationFrame(()=>$('#centralWishFormSection')?.scrollIntoView({behavior:'smooth',block:'start'}))};
window.cancelCentralWishEdit=()=>{editingCentralWishId='';window.hcWishFormOpen=false;renderWishlists()};
window.deleteCentralWish=id=>{const i=wishlists.findIndex(x=>String(x.id)===String(id));if(i<0||!samePerson(currentUserName(),wishlists[i].owner)||!confirm('Deze wens verwijderen?'))return;wishlists.splice(i,1);save()};
...
// V1.3.116 - datum van een gelegenheid zonder lokale tijdzone naar ISO-week omzetten.
