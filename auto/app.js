...
function item(html,id,type,care=''){const edit=type==='fuel'?`<button class="edit" data-edit-fuel="${id}">Wijzig</button>`:'';return `<div class="item ${care}"><div class="grow">${html}</div><div class="item-actions">${edit}<button class="delete" data-del="${type}" data-id="${id}">Verwijder</button></div></div>`}
...
function resetFuelForm(){ $('fuelForm').reset();$('fEditId').value='';$('fDate').value=today();$('fSaveButton').textContent='Tankbeurt opslaan';$('fCancelEdit').hidden=true }
function editFuel(id){const x=data.fuel.find(v=>String(v.id)===String(id));if(!x)return;$('fEditId').value=x.id;$('fDate').value=x.date;$('fOdo').value=x.odo;$('fLiters').value=x.liters;$('fAmount').value=x.amount;$('fFull').checked=!!x.full;$('fSaveButton').textContent='Wijzigingen opslaan';$('fCancelEdit').hidden=false;$('fuelForm').scrollIntoView({behavior:'smooth',block:'start'})}
$('fuelForm').onsubmit=e=>{e.preventDefault();const id=$('fEditId').value;const next={id:id||uid(),date:$('fDate').value,odo:Number($('fOdo').value),liters:Number($('fLiters').value),amount:Number($('fAmount').value),full:$('fFull').checked};if(id){const i=data.fuel.findIndex(x=>String(x.id)===String(id));if(i>=0)data.fuel[i]=next;else data.fuel.push(next)}else data.fuel.push(next);save();resetFuelForm()};
$('fCancelEdit').onclick=resetFuelForm;
document.body.addEventListener('click',e=>{const edit=e.target.closest('[data-edit-fuel]');if(edit){editFuel(edit.dataset.editFuel);return}let b=e.target.closest('[data-del]');if(!b)return;let type=b.dataset.del,id=b.dataset.id;if(type==='reservations')window.removeAutoReservationFromPlanner?.(id);data[type]=data[type].filter(x=>x.id!==id);save()});
