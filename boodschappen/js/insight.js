...
function receiptControl(r){const b=receiptBreakdown(r),k=Number(r.koopzegels)||0,p=Number(r.koopzegelsPaid)||0,calc=+(b.boodschappen+b.huishouden+b.verzorging+b.huisdieren+b.overig+k-p).toFixed(2),diff=+(Number(r.total)-calc).toFixed(2);return{...b,calc,diff}}
...
const openCalc=()=>{const box=row.querySelector('.receipt-line-calculator');box.hidden=false;prefillReceiptCalc(row);updateReceiptCalcUi(row);box.scrollIntoView({behavior:'smooth',block:'nearest'})};
...
row.querySelector('.receipt-move-up').onclick=()=>{const prev=row.previousElementSibling;if(prev)wrap.insertBefore(row,prev)};
row.querySelector('.receipt-move-down').onclick=()=>{const next=row.nextElementSibling;if(next)wrap.insertBefore(next,row)};
