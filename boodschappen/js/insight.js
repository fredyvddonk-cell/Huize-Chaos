...
if(list.length){const normal=list.reduce((a,b)=>a+b,0);let paidItems=[...list],paid=normal;const promo={"1+1":[2,1],"2+1":[3,1],"2+2":[4,2],"2+3":[5,3]};if(promo[calc.type]){const[groupSize,freeCount]=promo[calc.type],freeN=Math.floor(list.length/groupSize)*freeCount,order=list.map((price,i)=>({price,i})).sort((a,b)=>a.price-b.price||a.i-b.i);for(let i=0;i<freeN;i++)paidItems[order[i].i]=0;paid=paidItems.reduce((a,b)=>a+b,0)}else if(calc.type==='percent'){const pct=parseReceiptNumber(calc.value);if(pct===null)return null;const factor=1-Math.max(0,Math.min(100,pct))/100;paidItems=list.map(v=>v*factor);paid=paidItems.reduce((a,b)=>a+b,0)}else if(calc.type==='euro'){const discount=parseReceiptNumber(calc.value);if(discount===null)return null;paid=Math.max(0,normal-discount);const factor=normal>0?paid/normal:0;paidItems=list.map(v=>v*factor)}else if(calc.type==='bundle'){const bundleQty=parseReceiptNumber(calc.bundleQty),bundlePrice=parseReceiptNumber(calc.bundlePrice);if(bundleQty===null||bundlePrice===null||bundleQty<=0||bundlePrice<0)return null;const full=Math.floor(list.length/bundleQty),rest=list.length%bundleQty,sorted=[...list].sort((a,b)=>b-a);paid=full*bundlePrice+sorted.slice(0,rest).reduce((a,b)=>a+b,0);const factor=normal>0?paid/normal:0;paidItems=list.map(v=>v*factor)}const discount=Math.max(0,normal-paid);return{normal:+normal.toFixed(2),paid:+paid.toFixed(2),discount:+discount.toFixed(2),discountPct:normal>0?+(discount/normal*100).toFixed(1):0,paidItems:paidItems.map(v=>+v.toFixed(2)),variablePrices:new Set(list.map(v=>v.toFixed(2))).size>1}}
if(qty===null||unit===null||qty<0||unit<0)return null;const normal=qty*unit;let paid=normal;switch(calc.type){case'1+1':paid=(Math.floor(qty/2)+qty%2)*unit;break;case'2+1':paid=(Math.floor(qty/3)*2+qty%3)*unit;break;case'2+2':paid=(Math.floor(qty/4)*2+qty%4)*unit;break;case'2+3':paid=(Math.floor(qty/5)*2+qty%5)*unit;break;case'percent':{const pct=parseReceiptNumber(calc.value);if(pct===null)return null;paid=normal*(1-Math.max(0,Math.min(100,pct))/100);break}case'euro':{const discount=parseReceiptNumber(calc.value);if(discount===null)return null;paid=Math.max(0,normal-discount);break}case'bundle':{const bundleQty=parseReceiptNumber(calc.bundleQty),bundlePrice=parseReceiptNumber(calc.bundlePrice);if(bundleQty===null||bundlePrice===null||bundleQty<=0||bundlePrice<0)return null;const full=Math.floor(qty/bundleQty),rest=qty%bundleQty;paid=full*bundlePrice+rest*unit;break}}const discount=Math.max(0,normal-paid);return{normal:+normal.toFixed(2),paid:+paid.toFixed(2),discount:+discount.toFixed(2),discountPct:normal>0?+(discount/normal*100).toFixed(1):0}}
...
function matchingReceiptRows(row){const key=normalizeProductName(row.querySelector('.receipt-line-name')?.value||'');if(!key)return[row];return[...document.querySelectorAll('.receipt-product-row')].filter(r=>normalizeProductName(r.querySelector('.receipt-line-name')?.value||'')===key)}
...
row.querySelectorAll('.receipt-line-calculator input,.receipt-line-calculator select').forEach(el=>el.addEventListener('input',()=>updateReceiptCalcUi(row)));
row.querySelector('.receipt-calc-apply').onclick=()=>applyReceiptCalc(row);
...
function openModal(r){const m=document.querySelector('#receiptModal');m.dataset.groceryBaseTotal='';m.dataset.originalTotal=r?.originalTotal??'';m.dataset.koopzegels=r?.koopzegels||0;m.dataset.statiegeld=r?.statiegeld||0;m.dataset.excludedPurchaseTotal='';document.querySelector('#receiptTitle').textContent=r?.status==='pending'?'Bon controleren':r?'Bon aanpassen':'Bon toevoegen';document.querySelector('#receiptEditId').value=r?.id||'';document.querySelector('#receiptStore').value=r?.store||'';document.querySelector('#receiptDate').value=r?.date||new Date().toISOString().slice(0,10);document.querySelector('#receiptTotal').value=r?String(r.total).replace('.',','):'';document.querySelector('#receiptNote').value=r?.note||'';document.querySelector('#receiptFile').value='';m.dataset.fileName=r?.fileName||'';document.querySelector('#receiptFileName').textContent=r?.fileName||'Geen bestand gekozen';setReadStatus('');const rows=document.querySelector('#receiptProductRows');rows.innerHTML='';(r?.lines?.length?r.lines:[{}]).forEach(addProductRow);setSource(r?.source||'digital');m.classList.add('open');m.setAttribute('aria-hidden','false')}
...
window.addEventListener('DOMContentLoaded',()=>{let form=document.querySelector('#receiptForm');if(!form)return;document.querySelector('#receiptCancel').onclick=close;document.querySelector('#receiptBack').onclick=close;document.querySelector('#receiptModal').onclick=e=>{if(e.target.id==='receiptModal')close()};document.querySelectorAll('.receipt-source').forEach(b=>b.onclick=()=>setSource(b.dataset.source));document.querySelector('#receiptAddLine').onclick=()=>addProductRow({});document.querySelector('#receiptFile').onchange=e=>{const file=e.target.files[0];document.querySelector('#receiptFileName').textContent=file?.name||document.querySelector('#receiptModal').dataset.fileName||'Geen bestand gekozen';if(file){resetReceiptRecognitionFields();showReceiptPreview(file);readReceiptFile(file)}};
...
form.onsubmit=e=>{e.preventDefault();if(readingReceipt)return;let all=receipts(),editingId=document.querySelector('#receiptEditId').value,id=editingId||String(Date.now()),lines=collectLines(),entered=parseFloat(document.querySelector('#receiptTotal').value.replace(',','.')),total=Number.isFinite(entered)?entered:lines.reduce((a,l)=>a+l.price,0),file=document.querySelector('#receiptFile').files[0],old=all.find(r=>String(r.id)===String(id)),obj={id,store:document.querySelector('#receiptStore').value.trim(),date:document.querySelector('#receiptDate').value,total,lines,note:document.querySelector('#receiptNote').value.trim(),source:receiptSource,fileName:file?.name||document.querySelector('#receiptModal').dataset.fileName||old?.fileName||'',originalTotal:Number(document.querySelector('#receiptModal').dataset.originalTotal)||old?.originalTotal||null,koopzegels:Number(document.querySelector('#receiptModal').dataset.koopzegels)||old?.koopzegels||0,statiegeld:Number(document.querySelector('#receiptModal').dataset.statiegeld)||old?.statiegeld||0,status:'approved'};const duplicate=isDuplicateReceipt(obj,all,editingId);if(duplicate&&!window.confirm(`Mogelijk is deze bon al toegevoegd:
