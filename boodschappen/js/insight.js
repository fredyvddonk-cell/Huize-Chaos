...
function deleteReceipt(id){const r=receiptById(id);if(!r)return;const label=`${r.store||'Deze bon'} van ${new Date(r.date+'T12:00:00').toLocaleDateString('nl-NL',{day:'numeric',month:'long',year:'numeric'})}`;if(!window.confirm(`${label} verwijderen?\n\nDe bon en de bijbehorende inzichtgegevens worden verwijderd. De voorraad en boodschappenlijst veranderen niet.`))return;saveReceipts(receipts().filter(x=>String(x.id)!==String(id)));closeReceiptView();render()}
...
document.querySelector('#receiptViewBack').onclick=closeReceiptView;viewModal.onclick=e=>{if(e.target.id==='receiptViewModal')closeReceiptView()};viewMenuButton.onclick=e=>{e.stopPropagation();viewMenu.hidden=!viewMenu.hidden;viewMenuButton.setAttribute('aria-expanded',String(!viewMenu.hidden))};document.addEventListener('click',e=>{if(!e.target.closest('.receipt-view-menu-wrap')&&viewMenu&&!viewMenu.hidden){viewMenu.hidden=true;viewMenuButton.setAttribute('aria-expanded','false')}});document.querySelector('#receiptViewEdit').onclick=()=>{const r=receiptById(viewModal.dataset.id);if(!r)return;closeReceiptView();openModal(r)};document.querySelector('#receiptViewDelete').onclick=()=>deleteReceipt(viewModal.dataset.id);
...
Toch opslaan?`))return;let i=all.findIndex(r=>String(r.id)===String(id));if(i>=0)all[i]=obj;else all.push(obj);saveReceipts(all);close();render()};});
})();
