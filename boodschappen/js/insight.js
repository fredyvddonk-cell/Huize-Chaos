...
const regularWeekGroceries=rs=>+rs.reduce((sum,r)=>sum+(r.lines||[]).reduce((lineSum,l)=>lineSum+(countsAsGroceries(l)&&!isLargeStockPurchase(l)?(+l.price||0):0),0),0).toFixed(2);
const largeStockWeekTotal=rs=>+rs.reduce((sum,r)=>sum+(r.lines||[]).reduce((lineSum,l)=>lineSum+(countsAsGroceries(l)&&isLargeStockPurchase(l)?(+l.price||0):0),0),0).toFixed(2);
const KEYWORDS={
...
function showCategory(cat,rs,fromHistory=false){if(!fromHistory)window.pushHuizeChaosState?.({hcPage:'insight',hcInsightCategory:cat});let lines=[];rs.forEach(r=>(r.lines||[]).filter(l=>l.category===cat).forEach(l=>lines.push({...l,date:r.date,store:r.store})));const detailTotal=lines.reduce((a,l)=>a+(+l.price||0),0);document.querySelector('#content').innerHTML=`<button class="insight-back" id="insightBack">‹ Terug</button><h1>${esc(cat)}</h1><div class="insight-category-total"><small>Totaal</small><strong>${money(detailTotal)}</strong></div><div class="insight-products">${lines.sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))||(+b.price||0)-(+a.price||0)).map(l=>`<div><span><strong>${esc(l.name)}</strong><small>${new Date(l.date+'T12:00:00').toLocaleDateString('nl-NL',{day:'numeric',month:'short'})} · ${esc(l.store)}</small></span><strong>${money(l.price)}</strong></div>`).join('')||'<div class="empty">Geen producten.</div>'}</div>`;document.querySelector('#insightBack').onclick=()=>{if(history.state?.hcInsightCategory)history.back();else render()}}
window.showHuizeChaosInsightCategory=(cat,fromHistory=false)=>showCategory(cat,filtered(),fromHistory);
...
function secondHalfPriceItems(list=[]){const cents=list.map(v=>Math.max(0,Math.round(v*100))),order=cents.map((c,i)=>({c,i})).sort((a,b)=>b.c-a.c||a.i-b.i),paid=[...cents];for(let pos=1;pos<order.length;pos+=2){const {c,i}=order[pos];const discount=Math.floor(c/2);paid[i]=c-discount}return paid.map(c=>c/100)}
...
${duplicate.store} · ${new Date(duplicate.date+'T12:00:00').toLocaleDateString('nl-NL',{day:'numeric',month:'long',year:'numeric'})} · ${money(duplicate.total)}`);return}let i=all.findIndex(r=>String(r.id)===String(id));if(i>=0)all[i]=obj;else all.push(obj);if(file)await saveReceiptFile(id,file);saveReceipts(all);close();render()}finally{receiptSaving=false;if(saveButton)saveButton.disabled=false}};document.querySelector('#receiptSave')?.addEventListener('click',saveReceipt);setTimeout(openSharedReceipt,0);});
})();
