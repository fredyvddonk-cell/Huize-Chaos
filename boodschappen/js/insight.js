...
function insightCloudData(){return{receipts:receipts(),budgetWeek:Number(localStorage.getItem('hc-budget-week')||0),budgetMonth:Number(localStorage.getItem('hc-budget-month')||0),categoryMemory:categoryMemory()}}
function applyInsightCloudData(data={}){if(Array.isArray(data.receipts))localStorage.setItem('hc-receipts-v1',JSON.stringify(data.receipts));if(data.budgetWeek!==undefined)localStorage.setItem('hc-budget-week',String(Number(data.budgetWeek)||0));if(data.budgetMonth!==undefined)localStorage.setItem('hc-budget-month',String(Number(data.budgetMonth)||0));if(data.categoryMemory&&typeof data.categoryMemory==='object')localStorage.setItem(CAT_MEMORY_KEY,JSON.stringify(data.categoryMemory));if(document.body.classList.contains('insight-page'))render()}
window.getHuizeChaosInsightData=insightCloudData;
window.applyHuizeChaosInsightData=applyInsightCloudData;
const esc=s=>String(s??'').replace(/[&<>'"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[m]));
const catFor=name=>{const key=normalizeProductName(name),n=String(name||'').toLowerCase();/* V1.3.67: vaste regel gaat voor oude categorieleerdata */if(/suikerklont|\bsuiker\b|zoetje/.test(n))return 'Ontbijt & lunch';const remembered=categoryMemory()[key];if(CATS.includes(remembered))return remembered;for(const [c,ks] of Object.entries(KEYWORDS))if(ks.some(k=>n.includes(k)))return c;return 'Maaltijden'};
