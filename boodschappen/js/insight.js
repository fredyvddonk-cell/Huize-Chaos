...
row.querySelector('.receipt-calc-done')?.addEventListener('click',()=>{if(row.dataset.bulkCalc==='1'){const selected=[...document.querySelectorAll('.receipt-product-row')].filter(r=>r.querySelector('.receipt-line-select')?.checked);selected.forEach(r=>{if(r!==row){['type','value','type2','value2'].forEach(k=>{const a=row.querySelector('.receipt-calc-'+k),b=r.querySelector('.receipt-calc-'+k);if(a&&b)b.value=a.value})}applyReceiptCalc(r)});delete row.dataset.bulkCalc}else applyReceiptCalc(row);const box=row.querySelector('.receipt-line-calculator');if(box)box.hidden=true;priceInput.focus()});
const dragHandle=row.querySelector('.receipt-drag-handle');
let receiptDragging=false;
const stopReceiptDrag=()=>{if(!receiptDragging)return;receiptDragging=false;row.classList.remove('receipt-row-dragging');document.body.classList.remove('receipt-reordering');try{if(dragHandle.hasPointerCapture?.(dragHandle._receiptPointerId))dragHandle.releasePointerCapture(dragHandle._receiptPointerId)}catch{}dragHandle._receiptPointerId=null};
dragHandle.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'&&e.button!==0)return;receiptDragging=true;dragHandle._receiptPointerId=e.pointerId;try{dragHandle.setPointerCapture?.(e.pointerId)}catch{}row.classList.add('receipt-row-dragging');document.body.classList.add('receipt-reordering');e.preventDefault()});
dragHandle.addEventListener('pointermove',e=>{if(!receiptDragging)return;const y=e.clientY,rows=[...wrap.querySelectorAll('.receipt-product-row')].filter(r=>r!==row);let before=null;for(const r of rows){const b=r.getBoundingClientRect();if(y<b.top+b.height/2){before=r;break}}if(before)wrap.insertBefore(row,before);else wrap.appendChild(row);e.preventDefault()});
dragHandle.addEventListener('pointerup',stopReceiptDrag);dragHandle.addEventListener('pointercancel',stopReceiptDrag);dragHandle.addEventListener('lostpointercapture',stopReceiptDrag);
...
Toch opslaan?`))return;let i=all.findIndex(r=>String(r.id)===String(id));if(i>=0)all[i]=obj;else all.push(obj);if(file)await saveReceiptFile(id,file);saveReceipts(all);close();render()};document.querySelector('#receiptSave')?.addEventListener('click',saveReceipt);setTimeout(openSharedReceipt,0);});
})();
