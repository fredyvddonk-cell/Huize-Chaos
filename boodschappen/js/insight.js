...
function updateReceiptCalcUi(row){const calc=receiptCalcFromRow(row),result=calcReceiptLine(calc),valueWrap=row.querySelector('.receipt-calc-value-wrap'),bundleWrap=row.querySelector('.receipt-calc-bundle-wrap'),valueLabel=row.querySelector('.receipt-calc-value-label'),out=row.querySelector('.receipt-calc-result');if(valueWrap)valueWrap.hidden=!(calc.type==='percent'||calc.type==='euro');if(bundleWrap)bundleWrap.hidden=calc.type!=='bundle';if(valueLabel)valueLabel.textContent=calc.type==='percent'?'Kortingspercentage':'Korting €';if(out)out.innerHTML=result?`Normaal ${money(result.normal)} · korting ${money(result.discount)}${result.discountPct?` (${String(result.discountPct).replace('.',',')}%)`:''} · <strong>betaald ${money(result.paid)}</strong>`:'Vul aantal en normale stukprijs in.';return result}
function applyReceiptCalc(row){const result=updateReceiptCalcUi(row);if(!result)return;const price=row.querySelector('.receipt-line-price');if(price)price.value=String(result.paid.toFixed(2)).replace('.',',');const modal=document.querySelector('#receiptModal');if(modal&&String(modal.dataset.groceryBaseTotal??'').trim()===''){let total=0;document.querySelectorAll('.receipt-product-row').forEach(r=>{const cat=r.querySelector('.receipt-line-cat')?.value||'',v=parseReceiptNumber(r.querySelector('.receipt-line-price')?.value);if(!NON_GROCERY_CATS.has(cat)&&v!==null)total+=v});const input=document.querySelector('#receiptTotal');if(input)input.value=String(total.toFixed(2)).replace('.',',')}recalculateReceiptGroceryTotal();updateReceiptSummary({total:parseReceiptNumber(document.querySelector('#receiptTotal')?.value)||0,lines:collectLines(),store:document.querySelector('#receiptStore')?.value,date:document.querySelector('#receiptDate')?.value})}
...
row.querySelector('.receipt-calc-toggle').onclick=()=>{const box=row.querySelector('.receipt-line-calculator');box.hidden=!box.hidden;if(!box.hidden)updateReceiptCalcUi(row)};
