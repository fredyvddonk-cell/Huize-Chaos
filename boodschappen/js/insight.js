...
const NON_GROCERY_CATS=new Set(['Verzorging','Niet-boodschappen']);
...
function insightCloudData(){return{receipts:receipts(),budgetWeek:Number(localStorage.getItem('hc-budget-week')||0),budgetMonth:Number(localStorage.getItem('hc-budget-month')||0),careBudgetWeek:Number(localStorage.getItem('hc-budget-care-week')||0),careBudgetMonth:Number(localStorage.getItem('hc-budget-care-month')||0),categoryMemory:categoryMemory()}}
function applyInsightCloudData(data={}){if(Array.isArray(data.receipts))localStorage.setItem('hc-receipts-v1',JSON.stringify(data.receipts));if(data.budgetWeek!==undefined)localStorage.setItem('hc-budget-week',String(Number(data.budgetWeek)||0));if(data.budgetMonth!==undefined)localStorage.setItem('hc-budget-month',String(Number(data.budgetMonth)||0));if(data.careBudgetWeek!==undefined)localStorage.setItem('hc-budget-care-week',String(Number(data.careBudgetWeek)||0));if(data.careBudgetMonth!==undefined)localStorage.setItem('hc-budget-care-month',String(Number(data.careBudgetMonth)||0));if(data.categoryMemory&&typeof data.categoryMemory==='object')localStorage.setItem(CAT_MEMORY_KEY,JSON.stringify(data.categoryMemory));if(document.body.classList.contains('insight-page'))render()}
...
function budgetTotals(rs){let groceries=0,care=0,outside=0;rs.forEach(r=>(r.lines||[]).forEach(l=>{const v=+l.price||0;if(isOutside(l))outside+=v;else if(isCare(l))care+=v;else groceries+=v}));return{groceries:+groceries.toFixed(2),care:+care.toFixed(2),outside:+outside.toFixed(2)}}
...
function recalculateReceiptGroceryTotal(){const modal=document.querySelector('#receiptModal');if(!modal)return;const base=Number(modal.dataset.groceryBaseTotal);if(!Number.isFinite(base))return;const excluded=currentExcludedPurchaseTotal(),total=+Math.max(0,base-excluded).toFixed(2);const input=document.querySelector('#receiptTotal');if(input)input.value=String(total.toFixed(2)).replace('.',',');modal.dataset.excludedPurchaseTotal=String(excluded);refreshReceiptAmountMeta();updateReceiptSummary({total,koopzegels:Number(modal.dataset.koopzegels||0),statiegeld:Number(modal.dataset.statiegeld||0),excludedPurchases:excluded,originalTotal:Number(modal.dataset.originalTotal),store:document.querySelector('#receiptStore')?.value,date:document.querySelector('#receiptDate')?.value,lines:Array.from(document.querySelectorAll('.receipt-product-row')).map(row=>({name:row.querySelector('.receipt-line-name')?.value||'',price:parseFloat((row.querySelector('.receipt-line-price')?.value||'').replace(',','.'))||0,category:row.querySelector('.receipt-line-cat')?.value||''}))})}
...
function resetReceiptRecognitionFields(){
  ...
  document.querySelector('#receiptTotal').value='';const meta=document.querySelector('#receiptAmountMeta');if(meta){meta.hidden=true;meta.innerHTML=''};const modal=document.querySelector('#receiptModal');if(modal){delete modal.dataset.originalTotal;delete modal.dataset.koopzegels;delete modal.dataset.statiegeld;delete modal.dataset.groceryBaseTotal;delete modal.dataset.excludedPurchaseTotal}const summary=document.querySelector('#receiptDesktopSummaryInfo')||document.querySelector('#receiptDesktopSummary');if(summary)summary.innerHTML='<h3>Herkende bon</h3><p class="receipt-summary-empty">Kies een bon om de gegevens te controleren.</p>';clearReceiptPreview();
...
form.onsubmit=e=>{e.preventDefault();if(readingReceipt)return;let all=receipts(),editingId=document.querySelector('#receiptEditId').value,id=editingId||String(Date.now()),lines=collectLines(),entered=parseFloat(document.querySelector('#receiptTotal').value.replace(',','.')),total=Number.isFinite(entered)?entered:lines.reduce((a,l)=>a+l.price,0),file=document.querySelector('#receiptFile').files[0],old=all.find(r=>String(r.id)===String(id)),obj={id,store:document.querySelector('#receiptStore').value.trim(),date:document.querySelector('#receiptDate').value,total,lines,note:document.querySelector('#receiptNote').value.trim(),source:receiptSource,fileName:file?.name||document.querySelector('#receiptModal').dataset.fileName||old?.fileName||'',originalTotal:Number(document.querySelector('#receiptModal').dataset.originalTotal)||old?.originalTotal||null,koopzegels:Number(document.querySelector('#receiptModal').dataset.koopzegels)||old?.koopzegels||0,statiegeld:Number(document.querySelector('#receiptModal').dataset.statiegeld)||old?.statiegeld||0};const duplicate=isDuplicateReceipt(obj,all,editingId);if(duplicate&&!window.confirm(`Mogelijk is deze bon al toegevoegd:
