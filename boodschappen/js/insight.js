...
c.querySelectorAll('[data-mode]').forEach(b=>b.onclick=()=>{mode=b.dataset.mode;offset=0;localStorage.setItem('hc-insight-mode',mode);localStorage.setItem('hc-insight-offset','0');render()});c.querySelectorAll('[data-shift]').forEach(b=>b.onclick=()=>{offset+=Number(b.dataset.shift);localStorage.setItem('hc-insight-offset',offset);render()});let bi=c.querySelector('#insightBudget');bi.onchange=()=>{let v=parseFloat(bi.value.replace(',','.'))||0;localStorage.setItem(mode==='week'?'hc-budget-week':'hc-budget-month',v);render()};c.querySelectorAll('.insight-cat').forEach(b=>b.onclick=()=>showCategory(b.dataset.cat,rs));c.querySelectorAll('.receipt-row-main').forEach(b=>b.onclick=()=>openReceiptView(receipts().find(r=>String(r.id)===b.dataset.id)));c.querySelectorAll('.receipt-row-menu-button').forEach(btn=>btn.onclick=e=>{e.stopPropagation();const wrap=btn.closest('.receipt-row-menu-wrap'),menu=wrap.querySelector('.receipt-row-menu'),open=!menu.hidden;c.querySelectorAll('.receipt-row-menu').forEach(m=>m.hidden=true);c.querySelectorAll('.receipt-row-menu-button').forEach(x=>x.setAttribute('aria-expanded','false'));menu.hidden=!open;btn.setAttribute('aria-expanded',String(open))});c.querySelectorAll('[data-row-action]').forEach(btn=>btn.onclick=e=>{e.stopPropagation();const wrap=btn.closest('.receipt-row-wrap'),id=wrap?.dataset.id||'';const menu=btn.closest('.receipt-row-menu');if(menu)menu.hidden=true;if(btn.dataset.rowAction==='edit')editReceipt(id);if(btn.dataset.rowAction==='delete')deleteReceipt(id)});}
