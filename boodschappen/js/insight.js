...
function showOutsideRegular(rs=filtered()){
  const lines=[];
  rs.forEach(r=>(r.lines||[]).filter(l=>countsAsGroceries(l)&&isLargeStockPurchase(l)).forEach(l=>lines.push({...l,date:r.date,store:r.store})));
  const total=+lines.reduce((a,l)=>a+(+l.price||0),0).toFixed(2);
  document.querySelector('#content').innerHTML=`<button class="insight-back" id="insightBack">‹ Terug</button><h1>Buiten reguliere boodschappen</h1><div class="insight-category-total"><small>Totaal</small><strong>${money(total)}</strong></div><div class="insight-products">${lines.sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))||(+b.price||0)-(+a.price||0)).map(l=>`<div><span><strong>${esc(l.name)}</strong><small>${new Date(l.date+'T12:00:00').toLocaleDateString('nl-NL',{day:'numeric',month:'short'})} · ${esc(l.store)}${l.category?` · ${esc(l.category)}`:''}</small>${l.memo?`<small>Memo: ${esc(l.memo)}</small>`:''}</span><strong>${money(l.price)}</strong></div>`).join('')||'<div class="empty">Geen producten buiten reguliere boodschappen.</div>'}</div>`;
  document.querySelector('#insightBack').onclick=()=>render();
}
