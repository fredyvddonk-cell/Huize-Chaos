...
function occasionShoppingData(){const week=shoppingWeekKey();try{return (JSON.parse(localStorage.getItem('huize-chaos-occasions-v1')||'[]')||[]).map(e=>({...e,shopping:materializeOccasionShopping(e).filter(x=>!x.done&&((x.buyWeek||eventWeekKey(e.date))===week))})).filter(e=>e.shopping.length)}catch(_){return []}}
...
function recipeShoppingRows(){const week=shoppingWeekKey(),stock=products||[];return recipeWeekPlans().filter(p=>p.week===week).flatMap(plan=>(plan.ingredients||[]).filter(i=>!i.done).map((i,index)=>{const m=inferShoppingMeta(i.ingredient),inHouse=stock.some(x=>x.status==='In huis'&&String(x.name||'').toLowerCase()===String(i.ingredient||'').toLowerCase());return inHouse?null:{...m,category:i.category||m.category,store:i.store||m.store,name:i.ingredient,qty:i.shoppingQty||[i.qty,i.unit].filter(Boolean).join(' '),sourceName:plan.title,sourceType:'recipe',planId:plan.id,index}}).filter(Boolean))}
