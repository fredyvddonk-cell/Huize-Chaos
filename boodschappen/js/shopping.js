...
function recipeStockCoverage(i){
  const all=products||[];
...
function recipeShoppingRows(){const week=shoppingWeekKey();return recipeWeekPlans().filter(p=>p.week===week).flatMap(plan=>(plan.ingredients||[]).map((i,index)=>({i,index})).filter(x=>!x.i.ordered&&x.i.shoppingSelected!==false).map(({i,index})=>{const m=inferShoppingMeta(i.ingredient),coverage=recipeStockCoverage(i);/* V1.3.116: een net afgevinkt receptingrediënt blijft zichtbaar als doorgestreepte boodschap, ook nadat Voorraad op In huis is gezet. */if(coverage.enough&&!i.done)return null;return {...m,category:i.category||m.category,store:i.store||m.store,name:i.ingredient,qty:coverage.enough?'':(coverage.shortage||[i.qty,i.unit].filter(Boolean).join(' ')),sourceName:plan.title,sourceType:'recipe',planId:plan.id,recipeId:plan.recipeId,servings:plan.servings,index,done:Boolean(i.done)}}).filter(Boolean))}
function sourceShoppingHtml(){const rows=[...occasionShoppingRows(),...recipeShoppingRows()];if(!rows.length)return '';const key=group==='store'?'store':'category',grouped={};rows.forEach(x=>(grouped[x[key]||'Overig']??=[]).push(x));return Object.entries(grouped).map(([title,items])=>`<section class="shopping-group"><div class="shopping-group-head"><span>${esc(title)}</span><span>${items.length}</span></div><div class="shopping-group-body">${items.map(x=>`<div class="item shopping-item source-shopping-item ${x.done?'done':''}" role="button" tabindex="0" onclick="openSourceShoppingEdit('${x.sourceType}','${x.sourceType==='occasion'?String(x.eventId).replace(/'/g,"\\'"):String(x.planId).replace(/'/g,"\\'")}',${x.index})"><input class="check" type="checkbox" aria-label="${esc(x.name)} gekocht" ${x.done?'checked':''} onclick="event.stopPropagation()" onchange="${x.sourceType==='occasion'?`markOccasionBought('${String(x.eventId).replace(/'/g,"\\'")}',${x.index},this.checked)`:`markRecipeIngredientBought('${x.planId}',${x.index},this.checked)`}"><div class="main"><div class="name source-shopping-name">${esc([x.qty,x.name].filter(Boolean).join(' '))}</div><div class="meta source-shopping-meta">${[x.store,x.sourceName].filter(Boolean).map(esc).join(' · ')}</div></div>${x.sourceType==='recipe'&&x.recipeId?`<button class="mini-recipe-button" type="button" title="Recept bekijken" aria-label="Recept bekijken" onclick="event.stopPropagation();openShoppingRecipe('${String(x.recipeId).replace(/'/g,"\\'")}', '${String(x.planId).replace(/'/g,"\\'")}', '${esc(x.servings||'')}')">R</button>`:''}</div>`).join('')}</div></section>`).join('')}
function ensureSourceEditDialog(){let d=document.getElementById('sourceShoppingEditDialog');if(d)return d;d=document.createElement('dialog');d.id='sourceShoppingEditDialog';d.innerHTML=`<form class="source-shopping-edit"><h2>Boodschap wijzigen</h2><p id="sourceEditName" class="source-edit-name"></p><label>Hoeveelheid<input id="sourceEditQty" autocomplete="off"></label><label>Winkel<select id="sourceEditStore"></select></label><label>Categorie<select id="sourceEditCategory"></select></label><p id="sourceEditOrigin" class="meta"></p><div class="actions"><button id="sourceEditRemove" type="button" class="danger-button" hidden>Verwijderen</button><button id="sourceEditCancel" type="button">Annuleren</button><button id="sourceEditSave" type="button" class="primary">Opslaan</button></div></form>`;document.body.appendChild(d);d.querySelector('#sourceEditCancel').onclick=()=>window.closeHuizeChaosOverlay?.('source-shopping-edit',d);d.addEventListener('cancel',e=>{e.preventDefault();window.closeHuizeChaosOverlay?.('source-shopping-edit',d)});return d}
window.openShoppingRecipe=(recipeId,planId='',servings='')=>{sessionStorage.setItem('hc-shopping-return-scroll',String(window.scrollY||0));sessionStorage.setItem('hc-shopping-return-week',shoppingWeekKey());localStorage.setItem('household-page','list');sessionStorage.setItem('hc-household-page-session','list');window.location.href=`../recepten/?recipe=${encodeURIComponent(recipeId)}&readonly=shopping&view=ingredients${planId?`&plan=${encodeURIComponent(planId)}`:''}${servings?`&servings=${encodeURIComponent(servings)}`:''}`};
window.openSourceShoppingEdit=(type,id,index)=>{const d=ensureSourceEditDialog();let row=type==='occasion'?occasionShoppingRows().find(x=>String(x.eventId)===String(id)&&x.index===Number(index)):recipeShoppingRows().find(x=>String(x.planId)===String(id)&&x.index===Number(index));if(!row)return;sourceEditName.textContent=row.name;sourceEditQty.value=row.qty||'';const storeValues=[...new Set(['Overig',...(stores||[]),...(products||[]).map(x=>x.store),row.store].filter(Boolean))].sort((a,b)=>a.localeCompare(b,'nl'));const categoryValues=[...new Set(['Overig',...(categories||[]),...(products||[]).map(x=>x.category),row.category].filter(Boolean))].sort((a,b)=>a.localeCompare(b,'nl'));sourceEditStore.innerHTML=storeValues.map(value=>`<option value="${esc(value)}">${esc(value)}</option>`).join('');sourceEditCategory.innerHTML=categoryValues.map(value=>`<option value="${esc(value)}">${esc(value)}</option>`).join('');sourceEditStore.value=row.store||'Overig';sourceEditCategory.value=row.category||'Overig';sourceEditOrigin.textContent=`Herkomst: ${row.sourceName}`;sourceEditRemove.hidden=type!=='recipe';sourceEditRemove.onclick=()=>{if(type==='recipe'&&confirm(`${row.name} van de boodschappenlijst verwijderen?`)){removeRecipeIngredientFromShopping(id,Number(index));window.closeHuizeChaosOverlay?.('source-shopping-edit',d)}};sourceEditSave.onclick=()=>{saveSourceShoppingEdit(type,id,Number(index));window.closeHuizeChaosOverlay?.('source-shopping-edit',d)};window.openHuizeChaosOverlay?.('source-shopping-edit',d)}
window.saveSourceShoppingEdit=(type,id,index)=>{const qty=sourceEditQty.value.trim(),store=sourceEditStore.value.trim(),category=sourceEditCategory.value.trim();if(type==='occasion'){let events=[];try{events=JSON.parse(localStorage.getItem('huize-chaos-occasions-v1')||'[]')||[]}catch(_){}const e=events.find(x=>String(x.id)===String(id));if(!e)return;const visible=materializeOccasionShopping(e).filter(x=>(x.buyWeekOverride?x.buyWeek===shoppingWeekKey():weekContainsDate(shoppingWeekKey(),e.date))),target=visible[index];if(!target)return;const key=String(target.text||'').toLowerCase();let actual=(e.shopping||[]).find(x=>String(x.text||'').toLowerCase()===key);if(!actual){actual={...target};(e.shopping??=[]).push(actual)}actual.qty=qty;actual.store=store;actual.category=category;e.shoppingCreated=true;localStorage.setItem('huize-chaos-occasions-v1',JSON.stringify(events));window.dispatchEvent(new Event('huize-chaos-occasions-changed'));window.syncHuizeChaosOccasions?.(events)}else{const plans=recipeWeekPlans(),p=plans.find(x=>String(x.id)===String(id));if(!p||!p.ingredients?.[index])return;p.ingredients[index].shoppingQty=qty;p.ingredients[index].store=store;p.ingredients[index].category=category;p.changedAt=Date.now();localStorage.setItem('huize-chaos-recipe-weeks-v1',JSON.stringify(plans));window.dispatchEvent(new Event('huize-chaos-recipe-weeks-changed'))}render()}
...
window.markRecipeIngredientBought=(planId,index,checked)=>{
  ...
  if(checked){
    ...
    if(name){
      let stock=window.findExistingHuizeChaosProductByName?.(name)||(products||[]).find(x=>recipeIngredientMatchesProduct(name,x));
      if(stock){
...
function renderShopping(allProducts) {
  ...
  const sourceRows=[...occasionShoppingRows(),...recipeShoppingRows()];
  const done = arr.filter(x => x.done).length + sourceRows.filter(x=>x.done).length;
  const occasionCount=sourceRows.length;
  ...
  $('#clearDone').textContent = done ? `Gekochte producten verwijderen (${done})` : 'Gekochte producten verwijderen';
...
function bindShoppingEvents() {
  ...
  $('#collapseShoppingBtn').onclick = toggleAllShopping; const wl=$('#shoppingWeekLabel');if(wl){wl.textContent=weekLabel();wl.onclick=()=>{selectedShoppingWeek=currentShoppingWeekKey();localStorage.setItem('hc-shopping-selected-week',selectedShoppingWeek);render()};wl.title='Tik om naar de huidige week te gaan';wl.style.cursor='pointer'}$('#prevShoppingWeek').onclick=()=>shiftShoppingWeek(-1);$('#nextShoppingWeek').onclick=()=>shiftShoppingWeek(1);
  ...
  $('#clearDone').onclick = () => {
    ...
    // Verwijder ook afgevinkte recept- en gelegenheidsboodschappen pas na deze knop.
