...
// V1.3.105 - weekberekening volledig in UTC, zodat gelegenheden in een ander jaar correct aan de boodschappenweek gekoppeld blijven.
...
function recipeShoppingRows(){const week=shoppingWeekKey();return recipeWeekPlans().filter(p=>p.week===week).flatMap(plan=>(plan.ingredients||[]).map((i,index)=>({i,index})).filter(x=>!x.i.done&&!x.i.ordered&&x.i.shoppingSelected!==false).map(({i,index})=>{const m=inferShoppingMeta(i.ingredient),coverage=recipeStockCoverage(i);if(coverage.enough)return null;return {...m,category:i.category||m.category,store:i.store||m.store,name:i.ingredient,qty:i.shoppingQty||coverage.shortage||[i.qty,i.unit].filter(Boolean).join(' '),sourceName:plan.title,sourceType:'recipe',planId:plan.id,recipeId:plan.recipeId,index}}).filter(Boolean))}
function sourceShoppingHtml(){const rows=[...occasionShoppingRows(),...recipeShoppingRows()];if(!rows.length)return '';const key=group==='store'?'store':'category',grouped={};rows.forEach(x=>(grouped[x[key]||'Overig']??=[]).push(x));return Object.entries(grouped).map(([title,items])=>`<section class="shopping-group"><div class="shopping-group-head"><span>${esc(title)}</span><span>${items.length}</span></div><div class="shopping-group-body">${items.map(x=>`<div class="item shopping-item source-shopping-item"><input class="check" type="checkbox" aria-label="${esc(x.name)} gekocht" onclick="event.stopPropagation()" onchange="${x.sourceType==='occasion'?`markOccasionBought('${String(x.eventId).replace(/'/g,"\\'")}',${x.index},this.checked)`:`markRecipeIngredientBought('${x.planId}',${x.index},this.checked)`}"><div class="main"><button class="shopping-product-line" type="button" onclick="openSourceShoppingEdit('${x.sourceType}','${x.sourceType==='occasion'?String(x.eventId).replace(/'/g,"\\'"):String(x.planId).replace(/'/g,"\\'")}',${x.index})">${esc([x.qty,x.name].filter(Boolean).join(' '))}</button><div class="shopping-source-line"><span>${esc(x.sourceName||'')}</span>${x.sourceType==='recipe'&&x.recipeId?`<button class="recipe-link-small" type="button" onclick="event.stopPropagation();openShoppingRecipe('${String(x.recipeId).replace(/'/g,"\\'")}')">Recept</button>`:''}</div></div></div>`).join('')}</div></section>`).join('')}
window.openShoppingRecipe=(recipeId)=>{window.location.href=`../recepten/?recipe=${encodeURIComponent(recipeId)}&view=directions`};
