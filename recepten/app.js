...
function relevantStock(){return stockProducts().filter(x=>x.status==='In huis' && /^(Bewaarproducten \(voorraad\)|Groente|Diepvries|Vlees\s*\/?\s*vis|Vlees|Vis)$/i.test(String(x.category||''))).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'nl',{sensitivity:'base'}))}
