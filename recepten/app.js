...
const BASE=window.HUIZE_CHAOS_RECIPES||[];const PENDING_KEY='hc-recipe-pending-v1',CUSTOM_KEY='hc-recipe-custom-v1',META_KEY='hc-recipe-meta-v1',DELETED_KEY='hc-recipe-deleted-v1',RECIPE_WEEK_KEY='huize-chaos-recipe-weeks-v1';
...
function mergeWeekPlans(remote=[],local=[]){const merged=new Map();(remote||[]).forEach(plan=>merged.set(String(plan.id),plan));(local||[]).forEach(plan=>{const id=String(plan.id),current=merged.get(id);if(!current||Number(plan.changedAt||0)>Number(current.changedAt||0))merged.set(id,plan)});return [...merged.values()]}
...
async function syncCloud(){if(!user||!recipeRef||!fbSetDoc||!fbServerTimestamp)return;await fbSetDoc(recipeRef,{pending:pending(),custom:custom(),meta:recipeMeta(),deleted:deletedRecipes(),weekPlans:recipeWeekPlans(),updatedAt:fbServerTimestamp(),updatedBy:user.uid},{merge:false});setStatus('Gesynchroniseerd')}
...
function breadSplitRows(i,n,oldRows,gfPersons,totalPersons){
  ...
  if(regularPersons>0){
    ...
    const cov=stockCoverage(ing,prevFor('regular')?.stockProductId||'','regular');
    rows.push({mode:'regular',label:`Glutenvol · ${regularPersons} ${regularPersons===1?'persoon':'personen'}`,ingredient:ing,coverage:cov,options:sensitiveBreadStockOptions(baseName,true).filter(p=>!isGlutenFreeProduct(p)),shoppingSelected:prevFor('regular')?.shoppingSelected});
  }
  if(gfPersons>0){
    ...
    const cov=stockCoverage(ing,prevFor('gf')?.stockProductId||'','gf');
    rows.push({mode:'gf',label:`Glutenvrij · ${gfPersons} ${gfPersons===1?'persoon':'personen'}`,ingredient:ing,coverage:cov,options:sensitiveBreadStockOptions(baseName,true).filter(isGlutenFreeProduct),shoppingSelected:prevFor('gf')?.shoppingSelected});
  }
  return rows.map(row=>{
...
function showWeekPlanner(r,preferredWeek='',preferredGf=null){
  ...
  const refreshOrdered=()=>{
    ...
    orderedBox.querySelectorAll('[data-bread-split-choice]').forEach(choice=>choice.addEventListener('change',()=>{
      ...
      const coverage=stockCoverage(ing,choice.value,mode),row=choice.closest('.pasta-diet-row'),info=row?.querySelector('.stock-coverage'),shopping=row?.querySelector('[data-bread-shopping]');
      if(info){info.textContent=coverage.matched?(coverage.enough?coverage.label:`${coverage.label}${coverage.shortage?` · tekort ${[coverage.shortage.qty,coverage.shortage.unit].filter(Boolean).join(' ')}`:''}`):'Niet in huis';info.className=`stock-coverage ${coverage.enough?'enough':coverage.matched?'partial':'missing'}`}
