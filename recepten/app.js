...
function photoEditor(r){
  return `<div class="recipe-photo-editor"><span>Foto</span>${r.photo?`<img class="recipe-photo-preview" src="${esc(r.photo)}" alt="">`:''}<div class="recipe-photo-actions"><label class="btn recipe-photo-pick">${r.photo?'Foto vervangen':'Foto toevoegen'}<input id="recipePhotoInput" type="file" accept="image/*"></label>${r.photo?'<button class="btn" id="removeRecipePhoto" type="button">Foto verwijderen</button>':''}</div><small class="field-help">De foto wordt automatisch verkleind zodat Huize Chaos snel blijft.</small></div>`;
}
...
function showManualRecipeForm(){
  ...
  const bindManualPhoto=()=>{const input=detail.querySelector('#recipePhotoInput');if(input)input.onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{edited.photo=await resizeRecipePhoto(file);const wrap=detail.querySelector('.recipe-photo-editor');if(wrap)wrap.outerHTML=photoEditor(edited);bindManualPhoto()}catch(err){alert(err.message||'Foto kon niet worden toegevoegd.')}};detail.querySelector('#removeRecipePhoto')?.addEventListener('click',()=>{edited.photo='';const wrap=detail.querySelector('.recipe-photo-editor');if(wrap)wrap.outerHTML=photoEditor(edited);bindManualPhoto()})};bindManualPhoto();
...
// V1.4.83 - dubbele ingrediënthoeveelheden worden bij nieuwe recepten uit de bereidingsstappen verwijderd.
