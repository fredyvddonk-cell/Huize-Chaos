...
function allRecipes(){const deleted=new Set(deletedRecipes().map(String)),edits=new Map();BASE.forEach(r=>{if(deleted.has(String(r.id)))return;try{const x=JSON.parse(localStorage.getItem('hc_recipe_'+r.id)||'null');if(x)edits.set(String(r.id),x)}catch(_){}});return [...BASE.filter(r=>!deleted.has(String(r.id))).map(r=>edits.get(String(r.id))||r),...custom().filter(r=>!deleted.has(String(r.id)))]}
...
function isPieceIngredient(unit='',ingredient=''){const u=String(unit||'').trim().toLowerCase(),food=String(ingredient||'').toLowerCase();if(/^(stuks?|stuk|blik(?:je)?|zak(?:je)?|teen|tenen)$/.test(u))return true;if(u)return false;return /(?:^|\b)(komkommer|ui|uien|avocado|avocado's|paprika|courgette|ei|eieren|citroen|limoen|naanbrood|stokbrood|wraps?|pita(?:broodjes?)?|tortilla's?|bosje|krop)(?:\b|$)/i.test(food)}
...
function recipeCategory(r){const t=`${r.title||''} ${(r.ingredients||[]).map(i=>i.ingredient||'').join(' ')}`.toLowerCase();if(/wrap|taco|tortilla|burrito|tex.?mex|mexica|nacho|naan/.test(t))return 'Tex-Mex';if(/pasta|spaghetti|macaroni|lasagne|tagliatelle|penne/.test(t))return 'Pasta';if(/rijst|nasi|risotto|curry/.test(t))return 'Rijst';if(/aardappel|stamppot|puree/.test(t))return 'Aardappel';if(/soep|brood|bolletje/.test(t))return 'Soep/brood';if(/pizza|flammkuchen|plaat/.test(t))return 'Pizza';if(/oven|schotel/.test(t))return 'Ovenschotel';return 'Overig'}
...
function recipeStockFit(r){const inhouse=stockProducts().filter(p=>p.status==='In huis');let need=0,have=0;(r.ingredients||[]).forEach(i=>{const name=String(i.ingredient||'').toLowerCase();if(!name||/water|zout|peper|olie|boter|kruid|specer|bouillon/.test(name))return;need++;if(inhouse.some(p=>ingredientMatchesProduct(i,p)))have++});return {need,have,missing:Math.max(0,need-have),ratio:need?have/need:0}}
let smartRecipeCategory='Alles';
function renderSmartRecipePicker(){const box=document.querySelector('#smartRecipePicker');if(!box||recipeLibraryPanel?.classList.contains('hidden'))return;const plans=recipeWeekPlans().filter(p=>p.week===selectedMenuWeek),lates=lateShiftsForWeek(selectedMenuWeek),selected=new Set(plans.map(p=>String(p.recipeId))),all=allRecipes().filter(r=>!selected.has(String(r.id))),history=recipeWeekPlans(),counts=new Map(),last=new Map();history.forEach(p=>{const id=String(p.recipeId||'');counts.set(id,(counts.get(id)||0)+1);const ts=weekMondayFromKey(p.week||'').getTime();if(!last.has(id)||ts>last.get(id))last.set(id,ts)});const now=weekMondayFromKey(selectedMenuWeek).getTime();const stock=[...all].map(r=>({r,f:recipeStockFit(r)})).filter(x=>x.f.need>=2&&x.f.ratio>=.55).sort((a,b)=>b.f.ratio-a.f.ratio||a.f.missing-b.f.missing).slice(0,5);const fit=[...all].filter(r=>!lates.length||lateMealMode(r)!=='Vers maken').slice(0,5),old=[...all].filter(r=>last.has(String(r.id))&&now-last.get(String(r.id))>=6*7*86400000).sort((a,b)=>last.get(String(a.id))-last.get(String(b.id))).slice(0,4),frequent=[...all].filter(r=>(counts.get(String(r.id))||0)>0).sort((a,b)=>(counts.get(String(b.id))||0)-(counts.get(String(a.id))||0)).slice(0,4);const button=(r,sub='')=>`<button class="smart-recipe" type="button" data-smart-recipe="${esc(r.id)}"><span>${esc(r.title)}</span>${sub?`<small>${esc(sub)}</small>`:''}</button>`;const group=(title,rows)=>rows.length?`<section class="smart-group"><h3>${title}</h3><div>${rows.join('')}</div></section>`:'';const cats=['Alles','Pasta','Rijst','Aardappel','Tex-Mex','Ovenschotel','Soep/brood','Pizza','Overig'];box.innerHTML=`<div class="smart-picker"><div class="smart-picker-title"><strong>Recept kiezen</strong><small>Tik op een recept om het direct aan deze week toe te voegen.</small></div>${group('Past bij je voorraad',stock.map(x=>button(x.r,x.f.missing===0?'Alles in huis':`${x.f.missing} ${x.f.missing===1?'product':'producten'} nodig`)))}${group('Past bij deze week',fit.map(r=>button(r,lates.length?`Late dienst · ${lateMealMode(r)}`:'')))}${group('Al even niet gegeten',old.map(r=>button(r)))}${group('Vaak gegeten',frequent.map(r=>button(r)))}<div class="smart-categories">${cats.map(c=>`<button type="button" data-smart-cat="${esc(c)}" class="${smartRecipeCategory===c?'active':''}">${esc(c)}</button>`).join('')}</div><div class="smart-category-results">${all.filter(r=>smartRecipeCategory==='Alles'||recipeCategory(r)===smartRecipeCategory).slice(0,30).map(r=>button(r)).join('')}</div></div>`;box.querySelectorAll('[data-smart-recipe]').forEach(b=>b.onclick=()=>{const r=getRecipe(b.dataset.smartRecipe);if(r){quickAddRecipeToWeek(r);showRecipeModule('weekmenu')}});box.querySelectorAll('[data-smart-cat]').forEach(b=>b.onclick=()=>{smartRecipeCategory=b.dataset.smartCat;renderSmartRecipePicker()})}
