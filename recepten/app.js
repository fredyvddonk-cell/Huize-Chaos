// V1.4.45 - voorraadproduct opent eigen receptsuggesties; online zoeken blijft een rustige tweede stap.
...
let allRecipesCache=null,stockFitCache=new Map(),smartRecipeMode='all',smartRecipeLimit=24;
...
function renderSmartRecipePicker(){
  ...
  if(smartRecipeMode==='stock'){
    ...
    labelFor=r=>`${recipeCategory(r)}${r.servings?' · '+r.servings+' personen':''}`
  }
  ...
  box.innerHTML=`<div class="smart-picker calm"><div class="smart-picker-title"><strong>Recept kiezen</strong><small>Tik op een recept om het eerst te bekijken.</small></div><div class="smart-modes"><button type="button" data-smart-mode="all" class="${smartRecipeMode==='all'?'active':''}">Alles</button><button type="button" data-smart-mode="stock" class="${smartRecipeMode==='stock'?'active':''}">Past bij voorraad</button><button type="button" data-smart-mode="variation" class="${smartRecipeMode==='variation'?'active':''}">Variatie</button><button type="button" data-smart-mode="frequent" class="${smartRecipeMode==='frequent'?'active':''}">Vaak gegeten</button></div><div class="smart-categories">${cats.map(c=>`<button type="button" data-smart-cat="${esc(c)}" class="${smartRecipeCategory===c?'active':''}">${esc(c)}</button>`).join('')}</div><div class="smart-category-results">${visible.length?visible.map(button).join(''):'<div class="empty">Geen recepten binnen deze keuze.</div>'}</div>${rows.length>visible.length?`<button class="btn smart-more" id="smartMore" type="button">Meer recepten tonen</button>`:''}</div>`;
  box.querySelectorAll('[data-smart-recipe]').forEach(b=>b.onclick=()=>openRecipe(b.dataset.smartRecipe));
