...
function ingredients(r){const shown=scaledRecipe(r);return `<div class="panel">${(shown.ingredients||[]).length?shown.ingredients.map(x=>`<div class="ing"><span class="qty">${esc(x.qty)}</span><span class="unit">${esc(x.unit)}</span><span>${esc(x.ingredient)}</span>${x.memo?`<span class="memo">${esc(x.memo)}</span>`:''}</div>`).join(''):'<div class="empty">Nog geen ingrediënten.</div>'}</div>`}
function showView(view){
  ...
  const servingsBox=view==='edit'?'':`<div class="recipe-serving-control"><label>Aantal personen <input id="displayServings" type="number" min="1" inputmode="numeric" value="${esc(displayServings||r.servings||'')}"></label><small>Ingrediënten worden direct omgerekend. Het basisrecept blijft ongewijzigd.</small></div>`;
  detail.innerHTML=`<div class="detail-head"><div><h2>${esc(r.title)}</h2><small>${r.servings?`Basis: ${esc(r.servings)} personen`:''}</small>${sourceLine}</div><div class="actions"><button class="btn" id="backList">Terug</button></div></div>${servingsBox}<div class="tabs"><button class="tab ${view==='ingredients'?'active':''}" data-v="ingredients">Ingrediënten</button><button class="tab ${view==='directions'?'active':''}" data-v="directions">Bereiding</button><button class="tab ${view==='edit'?'active':''}" data-v="edit">Wijzigen</button></div><div id="recipeViewBody">${view==='ingredients'?ingredients(r):view==='directions'?`<div class="panel directions">${esc(r.directions||'Nog geen bereidingswijze.')}</div>`:editForm(r,false)}</div>`;
  ...
  if(servingInput){
    const applyServings=()=>{displayServings=String(Math.max(1,Number(servingInput.value)||1));if(view==='ingredients'){const body=detail.querySelector('#recipeViewBody');if(body)body.innerHTML=ingredients(r)}};
    servingInput.addEventListener('input',applyServings);servingInput.addEventListener('change',applyServings);
  }
...
// V1.3.102 - voorraad koppelen aan recepten
...
function pastaStockOptions(){return stockProducts().filter(p=>p.status==='In huis'&&pastaType(p.name))}
...
// V1.3.102 - recepten per week plannen (zonder dagen)
...
function stockCoverage(ingredient,preferredProductId='',glutenMode=''){
  let matches=stockProducts().filter(p=>p.status==='In huis'&&ingredientMatchesProduct(ingredient.ingredient,p));
...
function pastaSplitRows(i,n,prev,gfPersons,totalPersons){
  const regularPersons=Math.max(0,totalPersons-gfPersons), rows=[];
  if(regularPersons>0){const ing={...i,qty:splitQty(i.qty,regularPersons,totalPersons,i.unit,i.ingredient)},cov=stockCoverage(ing,prev?.regularStockProductId||'','regular');rows.push({mode:'regular',label:`Glutenvol · ${regularPersons} ${regularPersons===1?'persoon':'personen'}`,ingredient:ing,coverage:cov,options:pastaStockOptions().filter(p=>!isGlutenFreeProduct(p))})}
  if(gfPersons>0){const ing={...i,qty:splitQty(i.qty,gfPersons,totalPersons,i.unit,i.ingredient)},cov=stockCoverage(ing,prev?.gfStockProductId||'','gf');rows.push({mode:'gf',label:`Glutenvrij · ${gfPersons} ${gfPersons===1?'persoon':'personen'}`,ingredient:ing,coverage:cov,options:pastaStockOptions().filter(isGlutenFreeProduct)})}
  return rows.map(row=>{const c=row.coverage,info=c.matched?(c.enough?c.label:`${c.label} · tekort ${[c.shortage?.qty,c.shortage?.unit].filter(Boolean).join(' ')}`):'Niet in huis';return `<div class="pasta-diet-row"><strong>${esc(row.label)}</strong><span class="order-ingredient-line">${esc([row.ingredient.qty,row.ingredient.unit,'pasta'].filter(Boolean).join(' '))}</span><small class="stock-coverage ${c.enough?'enough':c.matched?'partial':'missing'}">${esc(info)}</small>${row.options.length?`<select class="stock-alternative-select" data-pasta-split-choice="${n}:${row.mode}">${row.options.map(p=>`<option value="${esc(p.id)}" ${String(c.product?.id)===String(p.id)?'selected':''}>${esc(p.name)} · ${esc([p.quantity,p.unit].filter(Boolean).join(' '))}</option>`).join('')}</select>`:`<small class="stock-coverage missing">Geen ${row.mode==='gf'?'glutenvrije':'glutenvolle'} pasta in huis</small>`}</div>`}).join('')
}
...
function showWeekPlanner(r){
  ...
  const refreshOrdered=()=>{
    ...
    orderedBox.querySelectorAll('[data-pasta-split-choice]').forEach(choice=>choice.addEventListener('change',()=>refreshOrdered()));
  };
  select.addEventListener('change',refreshOrdered);gfInput.addEventListener('input',()=>{gfInput.dataset.touched='1';refreshOrdered()});refreshOrdered();
  box.querySelector('#saveRecipeWeek').onclick=()=>{const week=select.value,existing=plans.find(x=>String(x.recipeId)===String(r.id)&&x.week===week),old=existing?.ingredients||[],gfPersons=Math.max(0,Math.min(Number(shown.servings)||1,Number(gfInput.value)||0)),total=Math.max(1,Number(shown.servings)||1),planned=[];(shown.ingredients||[]).forEach((i,n)=>{if(isGenericPasta(i.ingredient)){const reg=total-gfPersons;[['regular',reg,'pasta'],['gf',gfPersons,'glutenvrije pasta']].forEach(([mode,count,name])=>{if(!count)return;const ing={...i,qty:splitQty(i.qty,count,total,i.unit,i.ingredient)},sel=box.querySelector(`[data-pasta-split-choice=\"${n}:${mode}\"]`),cov=stockCoverage(ing,sel?.value||'',mode);planned.push({id:`${n}-${mode}`,qty:ing.qty||'',unit:i.unit||'',ingredient:name,memo:i.memo||'',done:false,ordered:false,stockEnough:Boolean(cov.enough),stockProductId:cov.product?.id??'',stockLabel:cov.matched?cov.label:'',shoppingQty:cov.shortage?[cov.shortage.qty,cov.shortage.unit].filter(Boolean).join(' '):'',store:cov.product?.store||'',category:cov.product?.category||''})});return}const coverage=stockCoverage(i),ordered=week===upcomingWeekKey()?Boolean(box.querySelector(`[data-plan-ordered=\"${n}\"]`)?.checked):false;planned.push({id:String(n),qty:i.qty||'',unit:i.unit||'',ingredient:i.ingredient||'',memo:i.memo||'',done:old[n]?.done||false,ordered,stockEnough:Boolean(coverage.enough),stockProductId:coverage.product?.id??'',stockLabel:coverage.matched?coverage.label:'',shoppingQty:coverage.shortage?[coverage.shortage.qty,coverage.shortage.unit].filter(Boolean).join(' '):'',store:coverage.product?.store||'',category:coverage.product?.category||''})});const plan={id:existing?.id||String(Date.now()),recipeId:String(r.id),title:r.title,week,servings:shown.servings||r.servings||'',gfPersons,ingredients:planned};if(existing)Object.assign(existing,plan);else plans.push(plan);localStorage.setItem(RECIPE_WEEK_KEY,JSON.stringify(plans));window.dispatchEvent(new Event('huize-chaos-recipe-weeks-changed'));alert(`${r.title} staat gepland voor week ${Number(week.slice(-2))}.`);box.remove()}
}
...
// V1.3.102 - Weekmenu-overzicht: recepten per week bekijken, verplaatsen en verwijderen
...
// V1.3.102 - slimme bulkinvoer voor handmatige recepten toegevoegd.


// V1.3.102 - handmatig recept toevoegen met slimme algemene bulkinvoer
