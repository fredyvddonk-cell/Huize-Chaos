...
function showView(view){
  ...
  if(servingInput){
    const applyServings=()=>{displayServings=String(Math.max(1,Number(servingInput.value)||1));if(view==='ingredients'){const body=detail.querySelector('#recipeViewBody');if(body)body.innerHTML=ingredients(r);ensureRecipeActions?.();updateOpenedWeekPlanServings?.(r)}};
...
// V1.3.105 - voorraad koppelen aan recepten
...
// V1.3.105 - recepten per week plannen (zonder dagen)
...
function showWeekPlanner(r){
  ...
  box.querySelector('#saveRecipeWeek').onclick=()=>{const week=select.value,existing=plans.find(x=>String(x.recipeId)===String(r.id)&&x.week===week),old=existing?.ingredients||[],gfPersons=Math.max(0,Math.min(Number(shown.servings)||1,Number(gfInput.value)||0)),total=Math.max(1,Number(shown.servings)||1),planned=[];(shown.ingredients||[]).forEach((i,n)=>{if(isGenericPasta(i.ingredient)){const reg=total-gfPersons;[['regular',reg,'pasta'],['gf',gfPersons,'glutenvrije pasta']].forEach(([mode,count,name])=>{if(!count)return;const ing={...i,qty:splitQty(i.qty,count,total,i.unit,i.ingredient)},sel=box.querySelector(`[data-pasta-split-choice=\"${n}:${mode}\"]`),cov=stockCoverage(ing,sel?.value||'',mode);const shoppingSelected=cov.enough?false:Boolean(box.querySelector(`[data-pasta-shopping=\"${n}:${mode}\"]`)?.checked);planned.push({id:`${n}-${mode}`,qty:ing.qty||'',unit:i.unit||'',ingredient:cov.product?.name||name,memo:i.memo||'',done:false,ordered:false,shoppingSelected,stockEnough:Boolean(cov.enough),stockProductId:cov.product?.id??'',stockLabel:cov.matched?cov.label:'',shoppingQty:cov.shortage?[cov.shortage.qty,cov.shortage.unit].filter(Boolean).join(' '):'',store:cov.product?.store||'',category:cov.product?.category||''})});return}const coverage=stockCoverage(i),ordered=[isoWeekKey(new Date()),upcomingWeekKey()].includes(week)?Boolean(box.querySelector(`[data-plan-ordered=\"${n}\"]`)?.checked):false;planned.push({id:String(n),qty:i.qty||'',unit:i.unit||'',ingredient:i.ingredient||'',memo:i.memo||'',done:old[n]?.done||false,ordered,stockEnough:Boolean(coverage.enough),stockProductId:coverage.product?.id??'',stockLabel:coverage.matched?coverage.label:'',shoppingQty:coverage.shortage?[coverage.shortage.qty,coverage.shortage.unit].filter(Boolean).join(' '):'',store:coverage.product?.store||'',category:coverage.product?.category||''})});const plan={id:existing?.id||String(Date.now()),recipeId:String(r.id),title:r.title,week,servings:shown.servings||r.servings||'',gfPersons,ingredients:planned};if(existing)Object.assign(existing,plan);else plans.push(plan);localStorage.setItem(RECIPE_WEEK_KEY,JSON.stringify(plans));window.dispatchEvent(new Event('huize-chaos-recipe-weeks-changed'));alert(`${r.title} staat gepland voor week ${Number(week.slice(-2))}.`);box.remove()}
}
function updateOpenedWeekPlanServings(r){
  if(!openedFromWeekMenu)return;
  const plans=recipeWeekPlans(),plan=plans.find(x=>String(x.recipeId)===String(r.id)&&x.week===selectedMenuWeek);
  if(!plan)return;
  const shown=scaledRecipe(r),total=Math.max(1,Number(shown.servings)||1),gfPersons=Math.max(0,Math.min(total,Number(plan.gfPersons)||0));
  const old=Array.isArray(plan.ingredients)?plan.ingredients:[], next=[];
  (shown.ingredients||[]).forEach((i,n)=>{
    if(isGenericPasta(i.ingredient)){
      const reg=total-gfPersons;
      [['regular',reg,'pasta'],['gf',gfPersons,'glutenvrije pasta']].forEach(([mode,count,name])=>{
        if(!count)return;
        const ing={...i,qty:splitQty(i.qty,count,total,i.unit,i.ingredient)}, prev=old.find(x=>String(x.id)===`${n}-${mode}`)||{};
        const cov=stockCoverage(ing,prev.stockProductId||'',mode);
        next.push({...prev,id:`${n}-${mode}`,qty:ing.qty||'',unit:i.unit||'',ingredient:cov.product?.name||prev.ingredient||name,shoppingSelected:cov.enough?false:prev.shoppingSelected!==false,stockEnough:Boolean(cov.enough),stockProductId:cov.product?.id??prev.stockProductId??'',stockLabel:cov.matched?cov.label:'',shoppingQty:cov.shortage?[cov.shortage.qty,cov.shortage.unit].filter(Boolean).join(' '):'',store:cov.product?.store||prev.store||'',category:cov.product?.category||prev.category||''});
      });
      return;
    }
    const prev=old.find(x=>String(x.id)===String(n))||old[n]||{},cov=stockCoverage(i,prev.stockProductId||'');
    next.push({...prev,id:String(n),qty:i.qty||'',unit:i.unit||'',ingredient:i.ingredient||'',stockEnough:Boolean(cov.enough),stockProductId:cov.product?.id??prev.stockProductId??'',stockLabel:cov.matched?cov.label:'',shoppingQty:cov.shortage?[cov.shortage.qty,cov.shortage.unit].filter(Boolean).join(' '):'',store:cov.product?.store||prev.store||'',category:cov.product?.category||prev.category||''});
  });
  plan.servings=String(total);plan.gfPersons=gfPersons;plan.ingredients=next;saveRecipeWeekPlans(plans);
}
const _showViewWeek=showView;showView=function(view){_showViewWeek(view);if(view==='ingredients'&&edited)ensureRecipeActions()};

// V1.3.105 - Weekmenu-overzicht: recepten per week bekijken, verplaatsen en verwijderen
...
// V1.3.105 - slimme bulkinvoer voor handmatige recepten toegevoegd.


// V1.3.105 - handmatig recept toevoegen met slimme algemene bulkinvoer
