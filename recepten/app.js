// V1.4.46 - receptdetail als eigen scherm, voorraadstatus hersteld, categorie/soort/tijd thuis bewerkbaar.
// V1.4.46 - voorraadproduct opent eigen receptsuggesties; online zoeken blijft een rustige tweede stap.
...
const RECIPE_CATEGORIES=['Pasta','Rijst','Aardappel','Tex-Mex','Ovenschotel','Soep/brood','Pizza','Overig'];
const RECIPE_TYPES=['Kip','Rund','Varken','Vis','Vegetarisch','Anders'];
...
function renderList(){
  ...
  list.innerHTML=a.length?a.map(r=>`<button class="recipe-card compact" data-id="${esc(r.id)}"><span><strong>${metaFor(r.id).favorite?'★ ':''}${esc(r.title)}</strong><small>${esc(recipeCategory(r))}${r.servings?' · '+esc(r.servings)+' personen':''}</small></span><span class="go">›</span></button>`).join(''):`<div class="empty">Geen recepten gevonden.</div>`;
...
function suggestionRecipes(plans,lates){const all=allRecipes(),history=recipeWeekPlans();const counts=new Map(),last=new Map();history.forEach(p=>{const id=String(p.recipeId||'');counts.set(id,(counts.get(id)||0)+1);const wk=weekMondayFromKey(p.week||'');const ts=wk.getTime();if(!last.has(id)||ts>last.get(id))last.set(id,ts)});const selected=new Set(plans.map(p=>String(p.recipeId)));const now=weekMondayFromKey(selectedMenuWeek).getTime();const frequent=[...all].filter(r=>!selected.has(String(r.id))).sort((a,b)=>(counts.get(String(b.id))||0)-(counts.get(String(a.id))||0)).slice(0,3);const old=[...all].filter(r=>{const ts=last.get(String(r.id));return !selected.has(String(r.id))&&ts&&now-ts>=6*7*86400000}).sort((a,b)=>(last.get(String(a.id))||0)-(last.get(String(b.id))||0)).slice(0,3);const fit=[...all].filter(r=>!selected.has(String(r.id))&&(lates.length>=2?/friet|pizza|wrap|soep|salade|snel|makkelijk|eenpans|naan/i.test(r.title||''):true)).slice(0,3);return {frequent,old,fit}}
...
function renderWeekContext(plans){const box=document.querySelector('#weekMenuContext');if(!box)return;const lates=lateShiftsForWeek(selectedMenuWeek),hutsel=hutselOpenItems(),types={};plans.forEach(p=>{const k=mealType(p.title);types[k]=(types[k]||0)+1});const dayFmt=d=>new Intl.DateTimeFormat('nl-NL',{weekday:'short',day:'numeric',month:'short'}).format(new Date(d+'T12:00:00'));const summary=Object.entries(types).map(([k,v])=>`${k} ${v}`).join(' · ')||'Nog geen vrije maaltijden gekozen';const sug=suggestionRecipes(plans,lates);const group=(title,arr)=>arr.length?`<div class="meal-suggestion-group"><strong>${title}</strong>${arr.map(r=>`<button type="button" data-suggest-recipe="${esc(r.id)}">${esc(r.title)}</button>`).join('')}</div>`:'';box.innerHTML=`<div class="week-fixed-meals"><strong>Vaste maaltijden</strong><span>Woensdag · friet met snacks</span><span>Zaterdag · soep met broodjes</span></div><details class="week-work-info"><summary>${lates.length} late ${lates.length===1?'dienst':'diensten'} deze week</summary>${lates.length?`<span>${lates.map(x=>dayFmt(x.date)).join(' · ')}</span>`:'<span>Geen late diensten in je werkrooster.</span>'}</details><div class="week-variation"><strong>Variatie</strong><span>${esc(summary)}</span></div>${hutsel.length?`<div class="week-hutsel"><strong>Hutsel Frutsel · ${hutsel.length} ${hutsel.length===1?'restje':'restjes'} opmaken</strong><span>Je kunt daardoor eventueel één maaltijd minder plannen.</span></div>`:''}<details class="meal-suggestions"><summary>Suggesties voor deze week</summary>${group('Vaak gegeten',sug.frequent)}${group('Al even niet gegeten',sug.old)}${group('Past bij deze week',sug.fit)}<small>Tip: houd het bij maximaal één nieuw recept als je weinig denkwerk wilt.</small></details>`;box.querySelectorAll('[data-suggest-recipe]').forEach(b=>b.addEventListener('click',()=>{const r=getRecipe(b.dataset.suggestRecipe);if(r)quickAddRecipeToWeek(r)}))}
...
function recipeCategory(r){
  ...
  if(/wrap|taco|tortilla|burrito|tex.?mex|mexica|nacho|naan/.test(t))return 'Tex-Mex';
  if(/pasta|spaghetti|macaroni|lasagne|tagliatelle|penne/.test(t))return 'Pasta';
  if(/rijst|nasi|risotto|curry/.test(t))return 'Rijst';
  if(/aardappel|stamppot|puree/.test(t))return 'Aardappel';
  if(/soep|brood|bolletje/.test(t))return 'Soep/brood';
  if(/pizza|flammkuchen|plaat/.test(t))return 'Pizza';
  if(/oven|schotel/.test(t))return 'Ovenschotel';
  return 'Overig'
}
function recipeProtein(r){const saved=String(r?.type||'').trim();if(RECIPE_TYPES.includes(saved))return saved;const t=`${r.title||''} ${(r.ingredients||[]).map(i=>i.ingredient||'').join(' ')}`.toLowerCase();if(/kip|kalkoen/.test(t))return 'Kip';if(/rund|gehakt|hamburger|biefstuk/.test(t))return 'Rund';if(/varken|ham|spek|pancetta|worst|salami/.test(t))return 'Varken';if(/zalm|tonijn|kabeljauw|vis|garnal|scampi/.test(t))return 'Vis';if(/tofu|tempeh|linzen|kikkererwt|vegetar|vega/.test(t))return 'Vegetarisch';return 'Anders'}
function recipeHomeTime(r){const saved=String(r?.homeTime||'').trim();return HOME_TIME_OPTIONS.includes(saved)?saved:''}
function variationScore(r,plans){const cats=new Map(),proteins=new Map();(plans||[]).forEach(p=>{const pr=getRecipe(p.recipeId)||p,c=recipeCategory(pr),prot=recipeProtein(pr);cats.set(c,(cats.get(c)||0)+1);proteins.set(prot,(proteins.get(prot)||0)+1)});const c=recipeCategory(r),prot=recipeProtein(r);let score=6-(cats.get(c)||0)*2-(proteins.get(prot)||0)*2;if(c==='Overig')score-=.5;if(prot==='Anders')score-=.25;return {score,category:c,protein:prot}}
...
function renderSmartRecipePicker(){
  ...
  if(smartRecipeMode==='stock'){
    ...
    labelFor=r=>{const v=variationScore(r,plans);return `Meer afwisseling · ${v.category} · ${v.protein}`}
    ...
    labelFor=r=>[recipeCategory(r),recipeProtein(r),recipeHomeTime(r),r.servings?r.servings+' personen':''].filter(Boolean).join(' · ')
  }
  ...
  const button=r=>`<button class="smart-recipe" type="button" data-smart-recipe="${esc(r.id)}"><span>${metaFor(r.id).favorite?'★ ':''}${esc(r.title)}</span><small>${esc(labelFor(r))}</small></button>`;
  box.innerHTML=`<div class="smart-picker calm"><div class="smart-picker-title"><strong>Recept kiezen</strong><small>Tik op een recept om het eerst te bekijken.</small></div><div class="smart-modes"><button type="button" data-smart-mode="all" class="${smartRecipeMode==='all'?'active':''}">Alles</button><button type="button" data-smart-mode="stock" class="${smartRecipeMode==='stock'?'active':''}">Past bij voorraad</button><button type="button" data-smart-mode="variation" class="${smartRecipeMode==='variation'?'active':''}">Variatie</button><button type="button" data-smart-mode="frequent" class="${smartRecipeMode==='frequent'?'active':''}">Vaak gegeten</button></div><div class="smart-categories">${cats.map(c=>`<button type="button" data-smart-cat="${esc(c)}" class="${smartRecipeCategory===c?'active':''}">${esc(c)}</button>`).join('')}</div><div class="smart-time-row"><span>Tijd thuis</span><div class="smart-times">${['Alles',...HOME_TIME_OPTIONS].map(t=>`<button type="button" data-smart-time="${esc(t)}" class="${smartRecipeTime===t?'active':''}">${esc(t)}</button>`).join('')}</div></div><div class="smart-category-results">${visible.length?visible.map(button).join(''):'<div class="empty">Geen recepten binnen deze keuze.</div>'}</div>${rows.length>visible.length?`<button class="btn smart-more" id="smartMore" type="button">Meer recepten tonen</button>`:''}</div>`;
  box.querySelectorAll('[data-smart-recipe]').forEach(b=>b.onclick=()=>openRecipe(b.dataset.smartRecipe));
...
function showManualRecipeForm(){
  ...
  <div class="panel bulk-recipe-panel"><label>Titel<input id="newRecipeTitle" placeholder="Naam van het recept"></label><label>Aantal personen<input id="newRecipeServings" type="number" min="1" inputmode="numeric" placeholder="4"></label><label>Categorie<select id="newRecipeCategory">${RECIPE_CATEGORIES.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select></label><label>Soort<select id="newRecipeType">${RECIPE_TYPES.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select></label><label>Tijd thuis<select id="newRecipeHomeTime"><option value="">Niet ingesteld</option>${HOME_TIME_OPTIONS.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('')}</select></label><label>Bron<input id="newRecipeSource" placeholder="Bijv. Picnic, Allerhande, eigen recept"></label><label>Bron / URL (optioneel)<input id="newRecipeSourceUrl" type="url" placeholder="https://…"></label>
