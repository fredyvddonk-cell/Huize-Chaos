...
// V1.3.101 - voorraad koppelen aan recepten
...
// V1.3.101 - recepten per week plannen (zonder dagen)
...
function stockCoverage(ingredient,preferredProductId=''){
  ...
  const pastaChoiceName=genericPasta && pastaType(productName) ? productName : '';
  const label=pastaChoiceName?`In huis: ${pastaChoiceName} · ${available}`:`In huis: ${available}`;
...
function showWeekPlanner(r){
  ...
  const refreshOrdered=()=>{
    ...
    orderedBox.innerHTML=plannerOrderRows(shown,existing,week);
...
// V1.3.101 - Weekmenu-overzicht: recepten per week bekijken, verplaatsen en verwijderen
...
// V1.3.101 - slimme bulkinvoer voor handmatige recepten toegevoegd.


// V1.3.101 - handmatig recept toevoegen met slimme algemene bulkinvoer
