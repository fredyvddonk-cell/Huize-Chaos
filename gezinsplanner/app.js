...
function renderTask(item){const done=isTaskDone(item);return `<article class="planner-item${done?' done':''}"><input class="check" type="checkbox" data-check="${item.id}" aria-label="Taak afronden" ${done?'checked':''}><div class="item-copy"><strong>${escapeHtml(item.title)}</strong>${item.note?`<small>${escapeHtml(item.note)}</small>`:''}${item.urgent?'<span class="urgent-badge">Urgent</span>':''}${deadlineInfo(item)}${item.repeat&&item.repeat!=='none'?`<span class="repeat-badge">${repeatLabel(item.repeat)}</span>`:''}</div>${item._big?bigActionButtons():actionButtons(item.id,true)}</article>`}
function renderUpcoming(item){const date=new Date(`${item.date}T12:00:00`);const label=new Intl.DateTimeFormat('nl-NL',{weekday:'short',day:'numeric',month:'short'}).format(date);return `<article class="planner-item"><span class="upcoming-date">${escapeHtml(label)}</span><div class="item-copy"><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.time?`${item.time} uur${item.note?' · '+item.note:''}`:item.note||'')}</small>${item.type==='task'?deadlineInfo(item):''}</div>${actionButtons(item.id)}</article>`}
...
function openModal(type,item=null){els.form.reset();els.id.value=item?.id||'';els.title.value=item?.title||'';els.time.value=item?.time||'';els.entryDate.value=item?.date||todayKey();els.hasDeadline.checked=Boolean(item?.deadline);els.deadline.value=item?.deadline||'';els.urgent.checked=Boolean(item?.urgent);els.repeat.value=item?.repeat||'none';els.note.value=item?.note||'';setType(type);els.modal.classList.add('open');els.modal.setAttribute('aria-hidden','false');setTimeout(()=>els.title.focus(),0)}
