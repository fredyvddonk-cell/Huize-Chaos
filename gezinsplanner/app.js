...
function renderAppointment(item){return `<article class="planner-item"><span class="time-badge">${escapeHtml(appointmentTime(item))}</span><div class="item-copy"><strong>${escapeHtml(item.title)}</strong>${item.note?`<small>${escapeHtml(item.note)}</small>`:''}${item.category==='work'?'<span class="work-badge">Werk</span>':''}${schoolBadge(item)}${privacyBadge(item)}</div>${actionButtons(item.id)}</article>`}
...
function renderUpcoming(item){const date=new Date(`${item.date}T12:00:00`);const label=new Intl.DateTimeFormat('nl-NL',{weekday:'short',day:'numeric',month:'short'}).format(date);const details=item.time?`${appointmentTime(item)} uur${item.note?' · '+item.note:''}`:item.note||'';return `<article class="planner-item"><span class="upcoming-date">${escapeHtml(label)}</span><div class="item-copy"><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(details)}</small>${item.category==='work'?'<span class="work-badge">Werk</span>':''}${schoolBadge(item)}${privacyBadge(item)}${item.type==='task'?deadlineInfo(item):''}</div>${item._bigUpcoming?'':actionButtons(item.id)}</article>`}
