...
document.getElementById('openFourWeek')?.addEventListener('click',()=>openFollowupWeek(6));
document.getElementById('openFollowupWeek')?.addEventListener('click',()=>openFollowupWeek(6));
document.getElementById('closeFourWeek').addEventListener('click',returnToEntryFromFollowup);
document.getElementById('previousFourWeek').addEventListener('click',()=>changeFourWeek(-7));
document.getElementById('resetFourWeek').addEventListener('click',()=>setFollowupWeeks(6));
document.getElementById('nextFourWeek').addEventListener('click',()=>changeFourWeek(7));
document.querySelectorAll('[data-followup-weeks]').forEach(button=>button.addEventListener('click',()=>setFollowupWeeks(Number(button.dataset.followupWeeks))));
fourWeekModal.addEventListener('click',event=>{if(event.target===fourWeekModal)returnToEntryFromFollowup()});
...
function weeksAheadMonday(baseKey,weeks){const base=new Date(`${baseKey}T12:00:00`);base.setDate(base.getDate()+(weeks*7));return mondayForDate(base)}
function captureEntryDraft(){return {id:els.id.value,type:els.type.value,category:els.category.value,title:els.title.value,time:els.time.value,endTime:els.endTime.value,date:els.entryDate.value,participants:els.participants.value,schoolTask:els.schoolTask.checked,hasDeadline:els.hasDeadline.checked,deadline:els.deadline.value,urgent:els.urgent.checked,private:els.private.checked,school:els.school.checked,repeat:els.repeat.value,note:els.note.value}}
function restoreEntryDraft(draft){if(!draft)return;els.id.value=draft.id||'';els.category.value=draft.category||'';els.title.value=draft.title||'';els.time.value=draft.time||'';els.endTime.value=draft.endTime||'';els.entryDate.value=draft.date||todayKey();els.participants.value=draft.participants||'';els.schoolTask.checked=Boolean(draft.schoolTask);els.hasDeadline.checked=Boolean(draft.hasDeadline);els.deadline.value=draft.deadline||'';els.urgent.checked=Boolean(draft.urgent);els.private.checked=Boolean(draft.private);els.school.checked=Boolean(draft.school);els.repeat.value=draft.repeat||'none';els.note.value=draft.note||'';setType('appointment')}
function openFollowupWeek(weeks=6){followupDraft=captureEntryDraft();followupBaseDate=els.entryDate.value||todayKey();els.modal.classList.remove('open');els.modal.setAttribute('aria-hidden','true');fourWeekMonday=weeksAheadMonday(followupBaseDate,weeks);document.querySelectorAll('[data-followup-weeks]').forEach(button=>{button.classList.toggle('primary',Number(button.dataset.followupWeeks)===weeks);button.classList.toggle('secondary',Number(button.dataset.followupWeeks)!==weeks)});renderFourWeek();history.replaceState({...history.state,hcPlannerPage:currentPlannerPage(),hcPlannerOverlay:'four-week'},'',location.href);fourWeekModal.classList.add('open');fourWeekModal.setAttribute('aria-hidden','false');fourWeekModal.setAttribute('tabindex','-1');try{fourWeekModal.focus({preventScroll:true})}catch(_){fourWeekModal.focus()}}
function setFollowupWeeks(weeks){fourWeekMonday=weeksAheadMonday(followupBaseDate||todayKey(),weeks);document.querySelectorAll('[data-followup-weeks]').forEach(button=>{button.classList.toggle('primary',Number(button.dataset.followupWeeks)===weeks);button.classList.toggle('secondary',Number(button.dataset.followupWeeks)!==weeks)});renderFourWeek()}
function returnToEntryFromFollowup(){fourWeekModal.classList.remove('open');fourWeekModal.setAttribute('aria-hidden','true');history.replaceState({...history.state,hcPlannerPage:currentPlannerPage(),hcPlannerOverlay:'entry'},'',location.href);els.modal.classList.add('open');els.modal.setAttribute('aria-hidden','false');restoreEntryDraft(followupDraft)}
function closeFourWeekModal(direct=false){if(direct){fourWeekModal.classList.remove('open');fourWeekModal.setAttribute('aria-hidden','true');return}returnToEntryFromFollowup()}
function changeFourWeek(days){fourWeekMonday.setDate(fourWeekMonday.getDate()+days);document.querySelectorAll('[data-followup-weeks]').forEach(button=>{button.classList.remove('primary');button.classList.add('secondary')});renderFourWeek()}
function renderFourWeek(){const end=new Date(fourWeekMonday);end.setDate(end.getDate()+6);const fmt=new Intl.DateTimeFormat('nl-NL',{day:'numeric',month:'long'});document.getElementById('fourWeekRange').textContent=`${fmt.format(fourWeekMonday)} t/m ${new Intl.DateTimeFormat('nl-NL',{day:'numeric',month:'long',year:'numeric'}).format(end)}`;const base=new Date(`${followupBaseDate||todayKey()}T12:00:00`);document.getElementById('followupBaseDate').textContent=`Gerekend vanaf ${new Intl.DateTimeFormat('nl-NL',{weekday:'long',day:'numeric',month:'long'}).format(base)}`;document.getElementById('fourWeekDays').innerHTML=Array.from({length:7},(_,index)=>{const date=new Date(fourWeekMonday);date.setDate(fourWeekMonday.getDate()+index);const key=localDateKey(date);const planned=entries.filter(item=>item.type==='appointment'&&item.date===key).sort((a,b)=>(a.time||'99:99').localeCompare(b.time||'99:99'));const rows=planned.map(item=>`<li><strong>${escapeHtml(item.time?appointmentTime(item):'Tijd onbekend')}</strong><span>${escapeHtml(item.title)}${item.category==='work'&&item.title!=='School'?' · Werk':''}</span></li>`).join('');return `<section class="availability-day ${planned.length?'has-appointments':'is-free'}"><div><strong>${new Intl.DateTimeFormat('nl-NL',{weekday:'long',day:'numeric',month:'short'}).format(date)}</strong><button type="button" data-plan-date="${key}">Kies deze dag</button></div>${rows?`<ul>${rows}</ul>`:'<p class="availability-empty">Geen afspraken</p>'}</section>`}).join('');document.querySelectorAll('[data-plan-date]').forEach(button=>button.addEventListener('click',()=>{const date=button.dataset.planDate;followupDraft={...followupDraft,date};fourWeekModal.classList.remove('open');fourWeekModal.setAttribute('aria-hidden','true');history.replaceState({...history.state,hcPlannerPage:currentPlannerPage(),hcPlannerOverlay:'entry'},'',location.href);els.modal.classList.add('open');els.modal.setAttribute('aria-hidden','false');restoreEntryDraft(followupDraft)}))}
...
function openPlannerOverlay(name,element){
  ...
  // V1.4.8 - laat een geopend formulier eerst als overzicht zien, zonder toetsenbord.
...
function shiftKey(item){if(!item)return '';const start=item.time??item.start??'';const end=item.endTime??item.end??'';for(const [key,shift] of Object.entries(SHIFTS))if(item.title===shift.title&&start===shift.start&&end===shift.end)return key;return 'custom'}
...
function updateRosterDraftStatus(){const choices=[...rosterDrafts.values()].filter(Boolean);document.getElementById('rosterDraftStatus').textContent=choices.length?'Wijzigingen bewaard':'Nog geen diensten tijdelijk ingevuld'}
