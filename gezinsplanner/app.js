...
function renderHousehold(){
  ...
  const selected=items.filter(item=>LONG_HOUSEHOLD_REPEATS.has(item.repeat)&&!isTaskDone(item)&&(householdTaskAddedThisWeek(item)||(householdTaskDueFromCompletion(item)&&!blockIds.has(item.id))));
  selected.forEach(item=>weekly.push(renderHouseholdTask(item)));
  const extra=items.find(item=>!blockIds.has(item.id)&&item.id!=='house-floor-downstairs'&&!LONG_HOUSEHOLD_REPEATS.has(item.repeat)&&!isTaskDone(item));
  ...
  els.householdLibrary.innerHTML=Object.entries(HOUSEHOLD_GROUPS).map(([key,label])=>{const repeat=key==='once'?'none':key;const group=items.filter(item=>(item.repeat||'none')===repeat);if(!group.length)return '';return `<details><summary>${escapeHtml(label)} <span>${group.length}</span></summary><div class="library-list">${group.map(item=>`<div class="library-task-row"><div><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.note||'')}</small></div>${LONG_HOUSEHOLD_REPEATS.has(item.repeat)?`<button type="button" class="library-add-button${householdTaskAddedThisWeek(item)?' added':''}" data-house-add="${escapeHtml(item.id)}" aria-label="${householdTaskAddedThisWeek(item)?'Al toegevoegd aan deze week':'Toevoegen aan deze week'}" ${householdTaskAddedThisWeek(item)?'disabled':''}>${householdTaskAddedThisWeek(item)?'✓':'+'}</button>`:''}</div>`).join('')}</div></details>`}).join('');
