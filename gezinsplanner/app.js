...
function renderHousehold(){
  ...
  if(floor&&!isTaskDone(floor))cards.push(renderHouseholdTask(floor));
  HOUSEHOLD_BLOCKS.forEach(block=>{const children=block.taskIds.map(id=>byId.get(id)).filter(item=>item&&!isTaskDone(item));if(children.length)cards.push(renderHouseholdBlock(block,children))});
...
function renderHouseholdBlock(block,children){return `<article class="planner-item household-block"><input class="check" type="checkbox" data-house-block="${block.id}" aria-label="Huishoudblok afronden"><div class="item-copy"><strong>${escapeHtml(block.title)}</strong><span class="repeat-badge">${escapeHtml(repeatLabel(block.repeat))}</span><details class="block-details"><summary>Bekijk wat erbij hoort</summary><ul>${children.map(item=>`<li>${escapeHtml(item.title)}</li>`).join('')}</ul></details></div></article>`}
...
function bindHouseholdActions(){document.querySelectorAll('[data-house-check]').forEach(input=>input.addEventListener('change',()=>{const item=entries.find(entry=>entry.id===input.dataset.houseCheck);if(item&&input.checked)completeHousehold(item)}));document.querySelectorAll('[data-house-block]').forEach(input=>input.addEventListener('change',()=>{if(!input.checked)return;const block=HOUSEHOLD_BLOCKS.find(item=>item.id===input.dataset.houseBlock);if(!block)return;block.taskIds.map(id=>entries.find(entry=>entry.id===id)).filter(item=>item&&!isTaskDone(item)).forEach(item=>{item.completedPeriods=item.completedPeriods||[];const key=periodKey(item.repeat);if(!item.completedPeriods.includes(key))item.completedPeriods.push(key)});persist();renderHousehold()}))}
