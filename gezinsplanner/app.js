...
function openDeleteModal(id){const item=entries.find(entry=>entry.id===id);if(!item)return;pendingDeleteId=id;document.getElementById('deleteTitle').textContent=`${item.type==='appointment'?'Afspraak':'Taak'} verwijderen?`;document.getElementById('deleteText').textContent=`Weet je zeker dat je ‘${item.title}’ wilt verwijderen? Dit kan niet ongedaan worden gemaakt.`;document.getElementById('deleteLinkedText').hidden=!(item.type==='appointment'&&entries.some(entry=>entry.linkedAppointmentId===id));deleteModal.classList.add('open');deleteModal.setAttribute('aria-hidden','false');setTimeout(()=>document.getElementById('cancelDelete').focus(),0)}
function closeDeleteModal(){pendingDeleteId='';deleteModal.classList.remove('open');deleteModal.setAttribute('aria-hidden','true')}
