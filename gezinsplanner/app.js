...
function openFourWeekModal(){fourWeekMonday=fourWeeksAheadMonday();renderFourWeek();fourWeekModal.classList.add('open');fourWeekModal.setAttribute('aria-hidden','false')}
function closeFourWeekModal(){fourWeekModal.classList.remove('open');fourWeekModal.setAttribute('aria-hidden','true')}
function changeFourWeek(days){fourWeekMonday.setDate(fourWeekMonday.getDate()+days);renderFourWeek()}
function renderFourWeek(){const end=new Date(fourWeekMonday);end.setDate(end.getDate()+6);document.getElementById('fourWeekRange').textContent=`${new Intl.DateTimeFormat('nl-NL',{day:'numeric',month:'long'}).format(fourWeekMonday)} t/m ${new Intl.DateTimeFormat('nl-NL',{day:'numeric',month:'long',year:'numeric'}).format(end)}`;document.getElementById('fourWeekDays').innerHTML=Array.from({length:7},(_,index)=>{const date=new Date(fourWeekMonday);date.setDate(date.getDate()+index);const key=localDateKey(date);const planned=entries.filter(item=>item.type==='appointment'&&item.date===key).sort((a,b)=>(a.time||'99:99').localeCompare(b.time||'99:99'));const rows=planned.length?planned.map(item=>`<li><strong>${escapeHtml(item.time?appointmentTime(item):'Tijd onbekend')}</strong><span>${escapeHtml(item.title)}${item.category==='work'?' · Werk':''}</span></li>`).join(''):'<li class="free-day">Geen dienst of afspraak ingevoerd</li>';return `<section class="availability-day"><div><strong>${new Intl.DateTimeFormat('nl-NL',{weekday:'long',day:'numeric',month:'short'}).format(date)}</strong><button type="button" data-plan-date="${key}">Plan afspraak</button></div><ul>${rows}</ul></section>`}).join('');document.querySelectorAll('[data-plan-date]').forEach(button=>button.addEventListener('click',()=>{const date=button.dataset.planDate;closeFourWeekModal();openModal('appointment');els.entryDate.value=date}))}
...
function showPlannerPage(page){
  ...
  if(page==='household'){renderHousehold();window.scrollTo({top:0,behavior:'smooth'})}
}
...
function renderHousehold(){
  ...
  HOUSEHOLD_BLOCKS.forEach(block=>{const children=block.taskIds.map(id=>byId.get(id)).filter(Boolean);if(!children.length)return;(block.repeat==='biweekly'?biweekly:weekly).push(renderHouseholdBlock(block,children))});
...
function renderHouseholdTask(item,keepVisible=false){const done=isTaskDone(item);return `<article class="planner-item compact-house-task${done?' done':''}${keepVisible?' household-main':''}"><input class="check" type="checkbox" data-house-check="${item.id}" aria-label="Huishoudtaak afronden" ${done?'checked':''}><div class="item-copy"><strong>${escapeHtml(item.title)}</strong>${keepVisible?'':`<small>${escapeHtml(repeatLabel(item.repeat))}</small>`}</div></article>`}
...
function renderAppointment(item){return `<article class="planner-item"><span class="time-badge">${escapeHtml(appointmentTime(item))}</span><div class="item-copy"><strong>${escapeHtml(item.title)}</strong>${item.note?`<small>${escapeHtml(item.note)}</small>`:''}${participantsLine(item)}${addedByLine(item)}${item.category==='work'?`<span class="work-badge">Werk${item.personName?' · '+escapeHtml(item.personName):''}</span>`:''}${schoolBadge(item)}${privacyBadge(item)}</div>${actionButtons(item.id,false,Boolean(item.time))}</article>`}
...
function bindItemActions(){
  ...
  document.querySelectorAll('[data-delete]').forEach(button=>button.addEventListener('click',()=>{if(!confirm('Dit item verwijderen?'))return;const id=button.dataset.delete;entries=entries.filter(item=>item.id!==id&&item.linkedAppointmentId!==id);persist();render()}));
