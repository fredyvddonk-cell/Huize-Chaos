...
function todayHouseholdItems(){return householdItems().filter(item=>TASK_LIST_HOUSEHOLD_REPEATS.has(item.repeat)&&!isTaskDone(item)&&(householdTaskAddedThisWeek(item)||householdTaskDueFromCompletion(item)))}
...
function renderHousehold(){
  ...
  const selected=items.filter(item=>!TASK_LIST_HOUSEHOLD_REPEATS.has(item.repeat)&&LONG_HOUSEHOLD_REPEATS.has(item.repeat)&&!isTaskDone(item)&&(householdTaskAddedThisWeek(item)||(householdTaskDueFromCompletion(item)&&!blockIds.has(item.id))));
  selected.forEach(item=>weekly.push(renderHouseholdTask(item)));
  const extra=items.find(item=>!blockIds.has(item.id)&&item.id!=='house-floor-downstairs'&&!TASK_LIST_HOUSEHOLD_REPEATS.has(item.repeat)&&!isTaskDone(item)&&!selected.includes(item));
  ...
  els.householdLibrary.innerHTML=Object.entries(HOUSEHOLD_GROUPS).map(([key,label])=>{const repeat=key==='once'?'none':key;const group=items.filter(item=>(item.repeat||'none')===repeat);if(!group.length)return '';return `<details><summary>${escapeHtml(label)} <span>${group.length}</span></summary><div class="library-list">${group.map(item=>`<div class="library-task-row"><div><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.note||'')}</small></div>${TASK_LIST_HOUSEHOLD_REPEATS.has(item.repeat)?`<button type="button" class="library-add-button${householdTaskAddedThisWeek(item)?' added':''}" data-house-add="${escapeHtml(item.id)}" aria-label="${householdTaskAddedThisWeek(item)?'Al toegevoegd aan de takenlijst':'Toevoegen aan de takenlijst'}" ${householdTaskAddedThisWeek(item)?'disabled':''}>${householdTaskAddedThisWeek(item)?'✓':'+'}</button>`:''}</div>`).join('')}</div></details>`}).join('');
...
function bindHouseholdActions(){
  ...
  document.querySelectorAll('[data-house-add]').forEach(button=>button.addEventListener('click',()=>{const item=entries.find(entry=>entry.id===button.dataset.houseAdd);if(!item||!TASK_LIST_HOUSEHOLD_REPEATS.has(item.repeat))return;if(isTaskDone(item))setHouseholdCompletion(item,false);item.manualWeekKey=periodKey('weekly');persist();render();renderHousehold()}));
}
...
function render(){renderHouseholdTime();renderStudy();renderRecurringChecklists();
  ...
  const tasks=[...todayEntries('task'),...todayHouseholdItems().map(item=>({...item,note:''}))].sort((a,b)=>Number(isTaskDone(a))-Number(isTaskDone(b))||Number(Boolean(b.urgent))-Number(Boolean(a.urgent))||(a.deadline||'9999-12-31').localeCompare(b.deadline||'9999-12-31')||a.createdAt-b.createdAt);const big=currentBigChore();if(big)tasks.push(big);
...
window.replaceHuizeChaosPlannerEntries=next=>{const obsoleteIds=new Set(['house-weekly-0','house-weekly-1','house-weekly-2','house-weekly-3','house-weekly-4']);entries=normalizePlannerEntries(next.filter(item=>!obsoleteIds.has(item.id)));localStorage.setItem(STORAGE_KEY,JSON.stringify(entries));render()};
