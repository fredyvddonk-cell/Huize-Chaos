...
function normalizeMaintenanceEntries(items){return items.map(item=>item.note==='Onderhoudsmelding van apparaat'?{...item,category:''}:item)}
function loadEntries(){try{return normalizeMaintenanceEntries(JSON.parse(localStorage.getItem(STORAGE_KEY))||[])}catch{return []}}
...
async function shareWasteReminder(){const schedule=wasteReminderForDate();if(!schedule)return;const now=new Date();const friday=new Date(now);friday.setHours(12,0,0,0);friday.setDate(now.getDate()+(5-now.getDay()));const date=new Intl.DateTimeFormat('nl-NL',{day:'numeric',month:'long'}).format(friday);const instruction=now.getDay()===4?schedule.lastDay:schedule.regular;const text=`${schedule.type} wordt vrijdag ${date} opgehaald. ${instruction}.`;if(navigator.share){try{await navigator.share({title:'Afval ophalen',text});return}catch(error){if(error.name==='AbortError')return}}window.open(`https://wa.me/?text=${encodeURIComponent(text)}`,'_blank','noopener')}
...
function renderAppointment(item){return `<article class="planner-item"><span class="time-badge">${escapeHtml(appointmentTime(item))}</span><div class="item-copy"><strong>${escapeHtml(item.title)}</strong>${item.note?`<small>${escapeHtml(item.note)}</small>`:''}${participantsLine(item)}${addedByLine(item)}${item.category==='work'?`<span class="work-badge">${item.title==='School'?'School':'Werk'}${item.personName?' · '+escapeHtml(item.personName):''}</span>`:''}${schoolBadge(item)}${privacyBadge(item)}</div>${actionButtons(item.id,false,Boolean(item.time),true)}</article>`}
...
async function shareAppointment(item){const date=new Intl.DateTimeFormat('nl-NL',{weekday:'long',day:'numeric',month:'long',year:'numeric'}).format(new Date(`${item.date}T12:00:00`));const names=participantNames(item);const lines=[item.title,date,item.time?`${appointmentTime(item)} uur`:'',names.length?`Voor: ${names.join(' en ')}`:'',item.note||''].filter(Boolean);const text=lines.join('\n');if(navigator.share){try{await navigator.share({title:item.title,text});return}catch(error){if(error.name==='AbortError')return}}window.open(`https://wa.me/?text=${encodeURIComponent(text)}`,'_blank','noopener')}
...
window.replaceHuizeChaosPlannerEntries=next=>{const obsoleteIds=new Set(['house-weekly-0','house-weekly-1','house-weekly-2','house-weekly-3','house-weekly-4']);entries=normalizeMaintenanceEntries(next.filter(item=>!obsoleteIds.has(item.id)));localStorage.setItem(STORAGE_KEY,JSON.stringify(entries));render()};
